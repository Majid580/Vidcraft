...
| **Current objective** | (1) Voice-to-text prompt input (the prior "next feature") is **now done** — ADR-022: a browser Web Speech API press-to-talk mic on the prompt composer that streams transcribed speech into the box, live-verified. (2) **Next actionable engineering task: INTEG-001/BACKEND-003 real queue wiring** — today's `POST /api/storyboards/:id/generate` is synchronous (ADR-014 stopgap), not the final async 202 shape; EVAL-001 (curate the fixed 50-prompt test set) is also fully unblocked and can go in parallel. (3) BACKEND-005's video tier stays **deliberately held** — do not retry Hugging Face against the depleted free-tier credits; resume only once the team supplies a paid `HUGGINGFACE_API_TOKEN` (no code change expected then). |
| **Overall status** | 🟢 **This session (ADR-022) implemented voice-to-text prompt input, live-verified.** Per the standing team request (was the sole `next_tasks` item): a press-to-talk mic on the prompt composer that streams transcribed speech into the prompt box. Built on the **browser-native Web Speech API** (`frontend/src/hooks/useSpeechRecognition.ts`, a reusable `{supported,listening,transcript,interim,error,start,stop,reset}` hook) rather than a hosted Groq `whisper-large-v3` endpoint — because `interimResults` streams words *live* as you speak (a record-then-POST Whisper flow can't) and it honors ADR-015's no-local-heavy-compute rule (in Chrome/Edge the audio is transcribed on the browser vendor's cloud: zero PC load, no local Whisper/torch), at **zero backend, zero API key, zero cost, and without touching the Groq free-tier quota**. `PromptComposer.tsx` gained a mic button inside the textarea (rendered only when `supported`, with the textarea auto-padding so text never runs under it), a live interim+final merge that appends spoken words *after* whatever was already typed (`mergeSpoken` collapses the seam to one space), a pulsing "Listening…" indicator, and friendly copy for the `not-allowed`/`no-speech`/`audio-capture` error codes. Interaction is **true press-and-hold** (hold the mic — or hold Space/Enter — to talk, release to stop; `setPointerCapture` guarded so a capture failure can't abort the press). The behaviour was factored into a shared `useDictation` hook + a self-contained `DictationMic` component, so the same mic now also sits **beside every clarification answer** (`ClarificationChat.tsx`), not just the first prompt — any text field can adopt it with one `<DictationMic value=… onValueChange=… />`. Unsupported browsers (Firefox/older Safari) simply don't show the mic; the typed fields are untouched. **Live-verified, not just written:** `tsc -b` + `oxlint` clean; driven in the in-app Chromium against the running Vite dev server — the composer mic renders as *Hold to dictate / Hold to talk* with the textarea padding at `56px`; a demo-mode run was driven all the way to the clarification chat, confirming exactly **one hold-to-talk mic beside each answer box**; pointer **and** keyboard (Space) hold gestures were dispatched on both mics and fired begin/end without crashing; and because the Browser pane blocks microphone access, the `not-allowed` path was exercised for real (the friendly *"Microphone access was blocked…"* message showed, `aria-pressed` returned to `false`, no stuck "listening" state). The currently-served module was verified clean by fetch (no stale symbols) with no Vite error overlay — note the Browser pane's console tool accumulates session history and kept showing earlier mid-edit HMR errors + one expected 502 (the real analyze failing with no backend, which is what triggers demo mode), so its buffer is not a reading of current state. The one thing the sandbox can't exercise is the actual streaming transcription (no microphone) — that needs a real mic on the dev machine. **What did not change:** BACKEND-005's video hold (ADR-021), the synchronous `/generate` stopgap (INTEG-001 still open), and the verified-task count (this is an additive feature outside the 35-task roadmap, like ADR-021's single-image mode). Files: `frontend/src/hooks/useSpeechRecognition.ts` (new), `frontend/src/hooks/useDictation.ts` (new), `frontend/src/components/DictationMic.tsx` (new), `frontend/src/components/PromptComposer.tsx`, `frontend/src/components/ClarificationChat.tsx`. Uncommitted, branch `rag-002-corpus`. |
| **Last updated** | 2026-08-11 |
| **Last updated by** | Claude (Opus 4.8) — see "Overall status" above. Wrote **ADR-022** (`PROJECT_ARCHITECTURE.md`) for the Web-Speech-API-over-hosted-Whisper decision; updated this file's Current objective / Overall status / §10 Next Recommended Actions, and `PROJECT_STATE.yaml`'s `last_updated_by`, `next_tasks` (voice-to-text → done; next = INTEG-001 async wiring), the ADR-022 `decisions` entry, and the Frontend Application component note. Code: `frontend/src/hooks/useSpeechRecognition.ts` (new — recognizer wrapper), `frontend/src/hooks/useDictation.ts` (new — press-and-hold gesture + field merge), `frontend/src/components/DictationMic.tsx` (new — reusable mic button), `frontend/src/components/PromptComposer.tsx` (composer mic + listening/error UI), `frontend/src/components/ClarificationChat.tsx` (per-answer mic). Two follow-up changes on the team's request in the same session: switched the interaction from click-to-toggle to true press-and-hold, and dropped the shared mic into the clarification chat. Prior: Claude (Sonnet 5) — ADR-021 (real per-shot image generation + single-image mode). Wrote **ADR-021** (`PROJECT_ARCHITECTURE.md`) covering both changes; updated FR-6's field table and added a new FR-6.1 subsection for single-image mode; updated Section 9.3 (`POST /api/storyboards/:id/generate` PROPOSED→IMPLEMENTED-with-deviation, added `POST /api/images`); updated the Section 20 BACKEND-005 roadmap row. Code: `backend/src/services/generationService.js` (new), `backend/src/routes/storyboards.js` (+`POST /:id/generate`), `backend/src/routes/images.js` (new), `backend/src/models/{Storyboard,Prompt}.js` (+`asset_url`/`error`/`on_hold` status; +`image_provider`/`image_url`), `backend/src/constants/renderProviders.js` (+`IMAGE_PROVIDERS`), `backend/src/app.js` (+`/media` static serving, +images route), `.gitignore` (+`backend/generated/`). Frontend: `lib/types.ts` (+`GenerationMode`/`ImageProvider`/`SingleImageResponse`), `lib/api.ts`/`lib/demo.ts` (+`generateSingleImage`/`demoSingleImage`), `StyleConfigurator/styleOptions.ts` (+`GENERATION_MODES`/`IMAGE_PROVIDER_IDS`/`StyleConfig.mode`), `StyleConfigurator/index.tsx` (+mode-toggle section, provider-list filtering, dynamic button label), `SingleImageView.tsx` (new), `FlowSteps.tsx` (+`resultLabel` prop), `App.tsx` (mode-based branching in `handleGenerate`), `vite.config.ts` (+`/media` proxy). Asked the user a scoping question first (verify-and-document only vs. actually wire real generation) before starting, since the two options differed by an order of magnitude in work and risk — team chose the larger scope. Prior: BACKEND-005 (external API adapter layer, image tier previously verified only at the direct-adapter-call level). |
...
NEXT 1 (next actionable engineering task on the critical path):
Task ID: INTEG-001 (real async queue-wiring slice)
Wire POST /api/storyboards/:id/generate onto the BACKEND-003 Bull/Redis queue
so generation is genuinely async (202 + GET /api/jobs/:id) instead of today's
synchronous stopgap (ADR-014). This is a strict superset of ADR-021's already-
working synchronous generation — same adapters, same output contract, re-hosted
behind a job — so nothing built in ADR-021 gets redone.
Dependencies:
AI-006, AI-008, REMOTION-003, BACKEND-005 (image tier), FRONTEND-002 — all
present; the image path is exercisable now, the video path stays held.
