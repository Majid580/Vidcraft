...
| **Current objective** | Three independent threads: (1) BACKEND-005's video tier stays **deliberately held** — do not retry Hugging Face against the depleted free-tier credits; resume only once the team supplies a paid `HUGGINGFACE_API_TOKEN` (no code change expected to be needed then); (2) INTEG-001/BACKEND-003 still need real queue wiring — today's `POST /api/storyboards/:id/generate` is synchronous, a stopgap per ADR-014's precedent, not the final async shape; (3) next feature, per direct team request from a prior session: voice-to-text prompt input (a press-and-hold/press-to-talk button that streams transcribed speech into the prompt box), still not started — this session's work was a different, more urgent team request (real image generation + single-image mode) that intervened first. |
| **Overall status** | 🟢 **This session (ADR-021) did two things, both live-verified, neither changing BACKEND-005's headline status.** First, per direct team instruction ("image generation is available on free tier on cloudflare do it and hold video generation until i provide paid api with higher limits"): real per-shot generation is now wired for the free image providers. `backend/src/services/generationService.js` dispatches each storyboard shot to `remotionService.renderShot()` (remotion) or `externalApiService.generateByProvider()` (pollinations/cloudflare) via a new `POST /api/storyboards/:id/generate`, persisting `shot.asset_url`/`status`/`error` and serving output at `/media` (`backend/generated/`, gitignored). `huggingface` shots are **not attempted** — held via a `HELD_PROVIDERS` map, returning `status:"on_hold"` with an explanatory message in ~60ms (confirmed no network call is made, unlike a real HF attempt's 30–120s+ cold start). Second, after seeing the result, the team asked why picking an image provider still produced a multi-shot storyboard rather than one image, and whether that should change. Clarified this is intentional per the project's own design (FR-9 concatenates every shot, any pathway, into one video; the evaluation study, EVAL-002/003, is built around comparing single-shot vs. multi-shot conditions) — not a bug. The team then asked for single-image generation as a **new, separate, additive mode**: `POST /api/images` (skips Screenwriter/storyboard entirely, generates one image from the enhanced prompt via `pollinations`/`cloudflare`) plus a frontend "What do you want to generate?" toggle in the Style Configurator, routing to a new `SingleImageView.tsx` that shows the original prompt next to the enhanced one alongside the image. **Live-verified, not just written:** backend curl round-trips — 4/4 real Cloudflare JPEGs generated and served via a real storyboard's `/generate` call, 3/3 real Pollinations JPEGs the same way, a huggingface storyboard's `/generate` call returning all 4 shots `on_hold` in 63ms (zero network calls), and a direct `POST /api/images` call returning a real 768×768 Pollinations JPEG with the style-token-merged enhanced prompt; a full real browser session (backend :5000 + ai-service :8000 WSL2 + real MongoDB) — real spaCy analysis → real Groq clarification Q&A → real Groq-clarified prompt → single-image mode selected → Cloudflare picked → a real 1024×1024 image rendered inline with zero console errors, then "Create another" reset back to Storyboard mode by default and a second real run (Pollinations, no clarification needed this time) confirmed the pre-existing multi-shot storyboard flow is unaffected. `tsc -b` clean. Test prompts/storyboards and generated files deleted after verification (`backend/generated/` is gitignored and disposable regardless). **What did not change:** BACKEND-005's Section 2 status (video tier still unmet, by design — see ADR-021); the queue (BACKEND-003) still has no real job type, so `/generate` is synchronous, same stopgap pattern as `POST /api/storyboards` (ADR-014); FR-7..FR-12 remain unimplemented; voice-to-text is still not started. Uncommitted, branch `rag-002-corpus`. |
| **Last updated** | 2026-08-11 |
| **Last updated by** | Claude (Sonnet 5) — see "Overall status" above for the full account. Wrote **ADR-021** (`PROJECT_ARCHITECTURE.md`) covering both changes; updated FR-6's field table and added a new FR-6.1 subsection for single-image mode; updated Section 9.3 (`POST /api/storyboards/:id/generate` PROPOSED→IMPLEMENTED-with-deviation, added `POST /api/images`); updated the Section 20 BACKEND-005 roadmap row. Code: `backend/src/services/generationService.js` (new), `backend/src/routes/storyboards.js` (+`POST /:id/generate`), `backend/src/routes/images.js` (new), `backend/src/models/{Storyboard,Prompt}.js` (+`asset_url`/`error`/`on_hold` status; +`image_provider`/`image_url`), `backend/src/constants/renderProviders.js` (+`IMAGE_PROVIDERS`), `backend/src/app.js` (+`/media` static serving, +images route), `.gitignore` (+`backend/generated/`). Frontend: `lib/types.ts` (+`GenerationMode`/`ImageProvider`/`SingleImageResponse`), `lib/api.ts`/`lib/demo.ts` (+`generateSingleImage`/`demoSingleImage`), `StyleConfigurator/styleOptions.ts` (+`GENERATION_MODES`/`IMAGE_PROVIDER_IDS`/`StyleConfig.mode`), `StyleConfigurator/index.tsx` (+mode-toggle section, provider-list filtering, dynamic button label), `SingleImageView.tsx` (new), `FlowSteps.tsx` (+`resultLabel` prop), `App.tsx` (mode-based branching in `handleGenerate`), `vite.config.ts` (+`/media` proxy). Asked the user a scoping question first (verify-and-document only vs. actually wire real generation) before starting, since the two options differed by an order of magnitude in work and risk — team chose the larger scope. Prior: BACKEND-005 (external API adapter layer, image tier previously verified only at the direct-adapter-call level). |
