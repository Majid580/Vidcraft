...
2026-08-13 (part 3 -- FR-2 clarification scaling, ADR-031; FR-10 narration, ADR-032)
- Reported from use: clarification asked TWO questions every time, whether
  the prompt was "a dog" or a full scene description.
- The cause was worse than a bad default. MAX_QUESTIONS was a hard 2 and
  generate_questions ended with questions[:2] -- a slice that discarded the
  ANALYZER'S OWN EVIDENCE. FR-1 already emits one flag per below-threshold
  dimension, so "a dog" produced FOUR flags naming four distinct gaps and
  two were thrown away before the user saw them. The reverse failure was
  equally real: one genuine gap still got two questions, the second padding.
- Budget now derives from the analysis: one per distinct flag, +1 for a
  prompt under 12 words (the analyzer cannot flag detail never written),
  +1 for an overall score under 40 (weak everywhere, not in one place),
  capped at 6. Zero flags still means zero questions and NO LLM call.
- The prompt-side change mattered as much: "ask AT MOST 2" is a ceiling, and
  a model satisfies a ceiling with two generic questions. Now "ask EXACTLY
  N", plus rules -- one gap per question, only what the camera can show,
  NEVER yes/no, and two or three worked examples each.
- Measured on real spaCy: "a dog" (15, 4 flags) 2 -> 6; "a man walks"
  (35, 3 flags) 2 -> 5; an 83-scoring prompt stays at 0 and skips the step.
  Live against real Groq, "a dog" returned six distinct non-overlapping
  questions with examples. Still ONE round -- what guarantees termination.

- FR-10 narration, both halves of the team's question answered by checking:
  EVERY video model in this project's reach returns SILENT video (Runway,
  Pika, Luma, Kling, Stable Video Diffusion, the HF text-to-video models).
  Native synchronised audio exists only in Veo 3 and Sora 2, neither in
  PROVIDER-001's tiers. So voiceover is added after generation.
- BEATS, NOT SECONDS. A per-second script cannot drive anything: one still
  per shot on every available pathway, and text-to-video models take one
  prompt per clip rather than instructions addressed to t=3s (keyframes are
  the finest control that exists). And a 4-second shot holds about two
  beats, not four -- equal slices invent boundaries and force padding. A
  per-second table is a VIEW renderable from beats; the reverse is not.
- WHAT IT SAYS: never inferred from generated pixels. The Narrator writes it
  from the storyboard, so picture and voice share one source -- the same
  shape as ADR-028's subtitles.
- HOW IT STAYS IN SYNC: the video is fitted to the audio. Constraining
  narration to an authored duration drifts, because speaking rate varies
  and error accumulates until the voice describes the wrong picture.
  Instead: synthesise first, measure, set the shot to the sum of its beats.
  Remotion takes duration_s as an INPUT, so drift is structurally absent
  rather than corrected -- and this needed NO timing change in the
  composition, because framesFor already read duration_s.
  Edge TTS reports spoken length itself (SentenceBoundary, 100ns ticks), so
  nothing is decoded or estimated; below a floor it RAISES rather than
  guessing, since a fallback estimate reintroduces the very drift removed.
- PROVIDERS, all three checked live: Edge TTS Tier 1 (hosted, so ADR-015
  holds -- nothing runs on the dev machine -- free, no key, no signup).
  Cloudflare Tier 2: melotts and aura-1 both exist (schema errors, not
  404s) but share one 10,000 neuron/day budget with image generation, spent
  on EVAL-004's frames. Groq OUT: playai-tts decommissioned, catalogue now
  Whisper only, which is speech-to-TEXT.
- Wiring: attachNarration runs AFTER generation (narrate only shots that
  survived -- a dropped shot occupies no time, so narrating it would push
  every later line ahead of its picture) and BEFORE assembly (it replaces
  duration_s). Audio to /media/{id}/narration-{shot}-{beat}.mp3; beats on
  the shot subdocument so GET /api/jobs/:id returns them for free;
  remotionService maps them across explicitly. Non-fatal at every level.
  Captions now follow BEAT boundaries when a shot is narrated.
- A REAL ROUNDING DEFECT found while wiring: rounding each beat to frames
  INDEPENDENTLY lets a shot's beats sum to a different count than the shot
  (two 1.11s beats -> 33+33=66; their 2.22s shot -> 67). One frame is
  enough to push an Audio clip past its Series.Sequence, desync captions
  from speech, and -- worst -- corrupt the frame total ADR-028's guard
  checks, silently disabling the check meant to catch this. Both consumers
  now round CUMULATIVELY within the shot, clamped to the shot's own count.
- Verified: 128/128 pytest (18 new), 10/10 caption-builder checks including
  that independent rounding genuinely WOULD have disagreed and that total
  frames are unchanged with narration on or off, 14/14 narration-service
  checks, and real synthesis end to end (3.212/2.487/2.462s onto a
  contiguous 8.162s timeline against 8.162s of measured speech).
- No task changed status; completion stays 32/36.

2026-08-13 (part 4 -- first render WITH SOUND, using none of the exhausted services)
- Asked what could be done with Groq exhausted, Cloudflare's neurons spent,
  and still no video-generation API. The answer turned out to be the single
  biggest open risk in the codebase: the FR-10 claim that a video renders
  with SYNCHRONISED voiceover had never actually been run. Closing it needs
  none of those three -- Edge TTS is free and keyless, the stills were
  already cached from EVAL-004, and Remotion and FFmpeg are local.
- Produced the first real render carrying a voiceover, behind a minimal
  stand-in for the /media mount (no Mongo, no Redis, no backend process).
  15/15 checks passed.
- THE NUMBERS: 14.144s H.264 against a 14.075s measured timeline; 423 frames,
  exactly what the composition reported; an AAC audio stream present.
- THE SYNC CLAIM WAS MEASURED, NOT ASSERTED. The audio track was extracted
  and its RMS sampled in the middle of every beat: ~1886-2348 in each of the
  four spoken beats, and 0 in the deliberately silent one. Speech lands
  precisely where the beat timeline says it should and nowhere else -- which
  is the whole ADR-032 argument, demonstrated rather than reasoned.
- CAPTIONS AGREED INDEPENDENTLY: four cues for four spoken beats, carrying
  the narration rather than the shot description, first cue at
  00:00:00.000, and the silent beat correctly showing as a GAP (cue 3 ends
  10.033s, cue 4 resumes 11.533s) while still consuming its time.
  postProcess reported no error and produced poster, subtitles and hardsub.
- Temporary harness deleted after use, per project convention.
- NOT exercised: the Narrator agent itself, which needs Groq (daily tokens
  spent finishing EVAL-003 that morning). Narration text was hand-authored
  for the harness, so the LLM authoring step remains covered by its unit
  tests while EVERYTHING DOWNSTREAM of it is now proven on real output.
  The first end-to-end run driven by the real Narrator still wants a fresh
  Groq day.
- No task changed status; completion stays 32/36.

2026-08-14 (Cloudflare render bug, ADR-033)
- Reported from a real 4-shot run: two shots rendered, two failed with
  `Provider 'cloudflare' failed: AiError: Bad input: Error: Additional or
  unevaluated properties '/seed, /negative_prompt' at '/' not allowed`.
- ROOT CAUSE: Workers AI validates every request body against THAT MODEL'S
  OWN published input schema, and the schemas are not uniform across the
  catalog — but cloudflareImage.js built one body shape for the whole
  provider. `@cf/black-forest-labs/flux-1-schnell`, the default since
  BACKEND-005, declares exactly {prompt, steps}. It had never accepted the
  FR-7 seed or negative prompt.
- WHY IT WENT UNNOTICED SO LONG — two separate reasons, both verified:
  (1) Cloudflare enforces the schema INCONSISTENTLY. Probed directly: the
      same illegal body returned 200 once and 400 twice in three consecutive
      calls. That is the whole explanation for "shots 1 and 3 failed, 2 and 4
      rendered" — the body was identical for all four shots.
  (2) The calls that DID succeed were silently wrong. flux has no seed and
      no negative_prompt, so it discarded both, while the adapter's return
      value still reported a seed. Every Cloudflare image this project has
      ever produced was unseeded and unguarded.
- FIX: per-MODEL parameter table (MODEL_INPUTS) + buildBody() in
  providers/cloudflareImage.js; only properties the target model declares are
  sent, and an unlisted model gets `prompt` alone rather than a 400. The
  table was checked mechanically against the live schema endpoint — 5/5
  models, no invented properties.
- DEFAULT MODEL CHANGED to `@cf/leonardo/phoenix-1.0`. Three
  schema-compatible candidates measured live, same prompt, 1024x1024, seed +
  full negative prompt:
      phoenix-1.0        ~3.1s   IDENTICAL bytes on a repeated seed
      sdxl-lightning     ~4.0s   differs; mislabels JPEG bytes image/png
      sdxl-base-1.0     ~10.8s   differs; 1.2MB PNG
  Determinism decided it, not speed: FR-12 and seedForShot both rest on a
  fixed seed reproducing a fixed image, and Phoenix is the only one of the
  three where that is true.
- A SECOND, INDEPENDENT DEFECT the fix exposed: with a model that finally
  READS the seed, Phoenix rejected roughly half of them (`'/seed' must be
  >= 0`). seedForShot mixed with Math.imul, which returns a SIGNED 32-bit
  result, and JS's `%` preserves the sign — negative seeds had been emitted
  since FR-7 was built, uncaught because the only provider that validated
  seeds was the one that discarded them. Fixed with `>>> 0`; verified
  non-negative, deterministic and per-shot-distinct across 40 samples.
- WHAT THIS INVALIDATES IN THE RECORD: DEMO-001's Cloudflare column (4/4)
  and every earlier Cloudflare still were produced WITHOUT the seed or the
  negative prompt in force. They remain valid evidence that the provider
  works and the images are usable; they are NOT evidence about FR-7
  continuity on Cloudflare. Pollinations reads both parameters, so ADR-027's
  continuity results — measured there — stand unchanged.
- VERIFICATION IS DELIBERATELY PARTIAL. Verified: parameter table against
  live schemas, per-model body construction, the seed fix. NOT verified: a
  live end-to-end 4-shot render. The account's 10,000 neuron/day free
  allocation was exhausted by the model comparison itself, so every request
  now fails on quota rather than on schema — which does establish that the
  bodies pass validation and reach inference. Re-run on a fresh day's
  allocation before treating the new default as proven in production.
  Recorded as a hold, not an automated retry (the standing convention for
  quota-bound work).
- No task changed status; completion stays 32/36.
```
...
**Outstanding re-verification, not a defect (2026-08-14, ADR-033):** the Cloudflare provider's request-body bug is fixed and its default model changed to `@cf/leonardo/phoenix-1.0`, but **a live 4-shot Cloudflare render has not been run against the fix.** The account's 10,000-neuron daily free allocation was exhausted by the model comparison that chose Phoenix, so every subsequent request returns `you have used up your daily free allocation of 10,000 neurons` — which does confirm the bodies now pass schema validation and reach inference, but is not the same as a rendered storyboard. The allocation resets daily and is shared across *everything* on the account (image generation, the CRITIC-001 vision model, the Tier 2 TTS models). **Next action:** on a fresh day, run one 4-shot Cloudflare storyboard end to end and confirm 4/4 plus byte-identical regeneration under a fixed seed; then update `DEMO_RUNBOOK.md`'s pathway table, which currently carries a warning note in place of a rehearsed figure. Remotion and Pollinations need no allocation and are unaffected.
...
NEXT 0 (small, quota-gated): re-verify Cloudflare on a fresh day's neurons.
  ADR-033 fixed the provider's request bodies and moved the default model to
  @cf/leonardo/phoenix-1.0, but the 10,000-neuron daily allocation ran out
  during the model comparison, so no 4-shot render has been done against the
  fix. Run one Cloudflare storyboard end to end; confirm 4/4 shots and that
  regenerating the same (storyboard, shot) returns byte-identical output now
  that the seed is honoured. Then replace DEMO_RUNBOOK.md's warning note with
  a real rehearsed figure. Costs one run, no new code.
