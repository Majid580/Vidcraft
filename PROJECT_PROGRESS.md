...
*(AI-008, PROVIDER-001, EVAL-001, and EVAL-002 moved to Section 2 — Verified. BACKEND-005 moved to Section 3 — In Progress. EVAL-003 is now fully unblocked — both dependencies satisfied.)*
...
NEXT 1 (fully unblocked): EVAL-003.
Task ID: EVAL-003
Run the multi-agent condition across the same 50 EVAL-001 prompts. Both
dependencies (INTEG-001, EVAL-001) are VERIFIED. Can reuse run_baseline.py's
runner shape (load EVAL-001, pace calls under the Groq free-tier cap,
isolate per-prompt failures, write one results JSON) but must call the full
orchestrator (ai-service/orchestrator/graph.py: screenwriter -> cinematographer
-> intent_check) instead of baseline.py, and needs its own module rather than
a shared abstraction (see ADR-024's reasoning). Before EVAL-004 can score
both conditions, also needs the media-generation question ADR-024 deferred
to be settled: whether/how each condition's output gets rendered so the
CLIPScore-style alignment metric has something to compare.
