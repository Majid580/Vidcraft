...
```text
Task ID: INTEG-001
Description: Full end-to-end wiring, frontend <-> backend <-> AI microservice <-> generation
Current state: BOTH the backend and frontend async-generation slices are DONE + independently verified
  (2026-08-11).
  Backend (ADR-023): the BACKEND-003 Bull/Redis queue now runs the real per-storyboard generation job --
  POST /api/storyboards/:id/generate enqueues one job and returns 202 {jobId,storyboardId,status}
  (in-flight dedup via Storyboard.job_id); the in-process worker (generationQueue.js processGeneration)
  renders/generates each shot via the unchanged generationService adapters, persisting per-shot
  status/asset_url/error to Mongo incrementally and reporting job.progress() 0-100; GET /api/jobs/:id
  polls {jobId,state,progress,storyboardId,failedReason?,shots[]}. Verified via an HTTP integration run
  (real Express+Bull+Redis+Mongo, generation stubbed) -- 11/11, deterministic across 3 runs.
  Frontend: App.tsx's handleGenerate now chains storyboard creation straight into POST /generate, then
  polls GET /api/jobs/:id every 1.5s (recursive setTimeout, cancelled on completion/reset/unmount);
  StoryboardView.tsx renders a generation progress bar, per-shot status pills
  (pending/processing/completed/failed/on_hold), and the finished asset_urls from /media (img for
  stills, video for .mp4), plus a Regenerate button. lib/{api,types,demo}.ts gained
  startGeneration()/getJob() and a client-side demo simulation of the same poll loop. Verified in-browser
  via demo mode (runs the identical App/StoryboardView code the real path uses): happy path (3 shots ->
  Done, progress 0->100, 3 assets rendered) and the video-held path (huggingface -> 3 shots On hold,
  amber message, 0 assets) both confirmed, zero console errors; tsc -b + oxlint clean.
Files: backend/src/queues/generationQueue.js, backend/src/routes/{storyboards,jobs}.js,
  backend/src/models/Storyboard.js (+job_id), backend/src/app.js; frontend/src/App.tsx,
  frontend/src/components/StoryboardView.tsx, frontend/src/lib/{api,types,demo}.ts
Dependencies: AI-006, AI-008, REMOTION-003, BACKEND-005 (image tier), FRONTEND-002 -- all present
What remains: (1) a REAL full-stack end-to-end confirmation run -- frontend UI -> real backend (:5000)
  -> ai-service (WSL2 :8000) -> real Mongo + Redis -> a live free provider -- to prove the two
  independently-verified halves (backend integration test; frontend demo-mode run) actually connect
  end to end; (2) make POST /api/storyboards (creation) async too; (3) optional WebSocket progress push
  (polling is the fallback used now, and is sufficient for the demo).
Current blocker: none (image/Remotion pathways). Video pathway stays a deliberate hold (BACKEND-005/ADR-021).
  Setup note for the real run: the backend defaults to REDIS_URL=redis://localhost:6379, but WSL2
  localhost-forwarding to Redis was flaky this session (only reachable via the WSL IP) -- confirm
  Redis connectivity before the run.
Next action: run the real full-stack end-to-end confirmation (or move to EVAL-001 in parallel), then
  mark INTEG-001 VERIFIED and move this row to Section 2.
```
...
*(FRONTEND-002 and FRONTEND-003 moved to Section 2 — Verified. INTEG-001 moved to Section 3 — In Progress: its backend async-generation slice is done + verified, ADR-023; the frontend generate+poll+render slice remains.)*

### MEDIUM (Target-tier outcomes)

*(AI-008 and PROVIDER-001 moved to Section 2 — Verified. BACKEND-005 moved to Section 3 — In Progress.)*
...
NEXT 1 (finishes INTEG-001):
Task ID: real full-stack end-to-end confirmation run
The backend async slice (ADR-023, HTTP integration test) and the frontend
generate+poll+render slice (demo mode) are each independently verified, but
have not yet been run TOGETHER against the real stack. Run: frontend UI ->
real backend (:5000) -> ai-service (WSL2 :8000) -> real Mongo + Redis -> a live
free provider (Pollinations/Cloudflare image, or Remotion) -> asset rendered in
the browser. This is the actual remaining closeout item before marking
INTEG-001 VERIFIED. Setup note: REDIS_URL defaults to localhost:6379, but
WSL2 localhost-forwarding to Redis was flaky this session (reachable only via
the WSL IP) -- confirm connectivity first. Also still open under INTEG-001:
make POST /api/storyboards (creation) async too; optional WebSocket progress
push (polling is the fallback used now, and is sufficient for the demo).
Dependencies:
Backend async endpoints (done, ADR-023) + frontend generate/poll/render UI
(done) + FRONTEND-002/003 (done).

NEXT 2 (fully unblocked, can run in parallel):
Task ID: EVAL-001
Curate the fixed 50-prompt test set. No dependencies — unblocked since project
start. Feeds EVAL-002/003 (single-shot baseline vs multi-agent, OBJ-8).
```
