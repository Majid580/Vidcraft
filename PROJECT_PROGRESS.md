...
| Field | Value |
|---|---|
| **Overall completion percentage** | **83.3%** — computed as (verified tasks) / (total tasks in the Section 20 roadmap of `PROJECT_ARCHITECTURE.md`) = **30 / 36**. Unchanged this session: INTEG-002 moved NOT_STARTED → **IN_PROGRESS** (FR-9 concatenation done, thumbnails/subtitles not), which doesn't add a verified task. CRITIC-001 (critic loop, FR-8) moved to VERIFIED the prior session — see Overall status below. BACKEND-005 and EVAL-003/EVAL-004 remain deliberately held per direct user instruction (see next row) — this is by team choice, not blockage. |
...
| **Current objective** | **This session (2026-08-12, team-reported defects):** the team reported that Remotion produced videos containing only text, and that storyboard images from Pollinations/Cloudflare were "almost completely different" from each other. Both were real and shared one root cause — the two halves of the pipeline were never connected (no `<Img>` anywhere in `remotion/src/`; `world_state` accepted but unused on the API generation path). Fixed both and implemented FR-9 assembly, recorded as **ADR-027** — see Overall status. **Next:** finish INTEG-002's remaining pieces (thumbnail extraction is the smallest — `ffmpeg` is already a verified dependency — then subtitles, then a call on cloud hosting vs. scoping it out for the demo), and live-verify the Cloudflare img2img reference-anchoring tier against real credentials. Carried over, unchanged: **BACKEND-005's video tier and EVAL-004 stay deliberately held** — BACKEND-005 because no video-generation API is currently available (Hugging Face free credits exhausted, no paid key), EVAL-004 because Groq's daily token quota is exhausted (blocks EVAL-003, which EVAL-004 depends on). Team directed moving to **CRITIC-001** instead, which is now **VERIFIED** (see Overall status). With CRITIC-001 done and the other two items deliberately held, there is currently no explicitly-requested unblocked work queued — next step is a team call on what to pick up next. `python -m evaluation.run_multiagent --resume` remains available manually for EVAL-003's remaining 23 prompts whenever Groq's daily quota allows. Also still open, non-blocking: making `POST /api/storyboards` (creation) itself async; optional WebSocket progress push. |
| **Overall status** | 🟢 **This session fixed two team-reported defects and implemented FR-9 assembly (ADR-027).** Both reported symptoms were real and shared one root cause — the pipeline's two halves were never connected. (1) *Remotion produced videos of only text*: `remotion/src/` contained **no `<Img>` element at all** — `WideShot`/`MediumShot`/`CloseUpShot` drew a background colour, a gradient and `<div>`s of caption text. Remotion is a compositor, not a generative model; nothing had ever handed it an asset. (2) *Storyboard images were unrelated to each other*: `generationService.generateShotAsset()` took `worldState` as its third parameter and **never used it** on the API pathway, passing a bare `shot.description` — so every shot was an independent draw sharing no subject/environment/palette, and AI-007's RAG-grounded `style_tokens` were computed, persisted, then discarded unread. FR-7's own table already specified the fix ("constructed once per storyboard, injected into every shot's generation call"); it had simply never been implemented for that pathway. Built `backend/src/services/continuityPrompt.js` (invariant `Subject:`/`Location:`/`Style:` block byte-identical across shots, deterministic per-shot seed, plus reference-image anchoring through `world_state.reference_image_url` — a schema field that existed and was read by nothing); widened the provider adapters to take a continuity options object; made the Remotion compositions image-first (`components/ShotLayer.tsx`, with the old motion-graphics card retained as the no-image fallback); and implemented **FR-9 assembly in Remotion rather than FFmpeg** (`StoryboardVideo` + `<Series>` + `render-storyboard.mjs`, invoked by the generation queue, persisted as `Storyboard.video_url`/`video_error`) — because an FFmpeg concat of stills yields a slideshow whereas every Remotion-assembled shot is already a moving shot, and it structurally closes **R-10** (one renderer, fixed 1920×1080/30fps, no cross-source concat to mismatch). **Two design corrections, both found by looking at real generated images rather than by reasoning about the API, and both worth keeping on record:** a single storyboard-wide seed (the obvious first choice, implemented first) is *wrong* — it pins the composition, not just the palette, and made a wide, a medium and a close-up all return the same front-facing portrait; and framing/action originally sat at the tail of the prompt where they lost to the invariant block, so they were moved to the front (diffusion text encoders weight earlier tokens more heavily), with terse camera strings like `"close-up, static"` expanded into descriptive framing phrases and motion words (`"slow dolly-in"`) deliberately dropped there to drive the Remotion camera move instead — framing to the image model, motion to the compositor. **Live-verified with no stubs:** three real Pollinations generations sharing one `world_state` produced the same subject, location and palette across all three with framing correctly varying wide → medium → close, assembled into one 1920×1080 H.264 MP4 of exactly 300 frames (the storyboard's 4+3+3s at 30fps), ffprobe-confirmed, frames sampled at 1.5s/5.0s/8.5s showing each shot's own still with its camera move applied and no text card anywhere. **Three real defects surfaced and were fixed during that verification:** helmet's default `Cross-Origin-Resource-Policy: same-origin` on `/media` blocked Remotion's headless browser from loading any still and failed every render with undecodable images (fixed in `backend/src/app.js`, scoped to `/media` alone — **a running backend must be restarted to pick this up**); `render-shot.mjs` ran `main()` at import time, making its `selectCompositionId` export unusable outside the CLI (main-module guard added); and the shot→composition taxonomy had to move to `remotion/src/shotTaxonomy.mjs` to be reachable from both the Node entry points and the browser bundle without duplicating the mapping. **Not verified:** the Cloudflare img2img reference-anchoring tier is implemented but has not been exercised against real credentials — it needs one real run to confirm the `image_b64` field and the response-format branching in `providers/cloudflareImage.js`. **Known residual drift, consistent with FR-7's declared best-effort scope:** a garment colour named in `world_state` can still shift between shots (observed black ↔ maroon for "red leather jacket"), and extreme close-ups on an *object* still render as tight portraits (provider portrait bias — the Remotion punch-in compensates by cropping in). Pixel-level identity lock stays out of reach for this provider set (needs IP-Adapter/InstantID-class conditioning neither offers), which is the pre-declared risk-table position, not a regression. INTEG-002 moved NOT_STARTED → **IN_PROGRESS**; completion unchanged at **30/36 (83.3%)**. Also corrected two `PROJECT_STATE.yaml` component entries that had drifted out of sync with the task list in the same file (Critic still read `NOT_IMPLEMENTED` while CRITIC-001 was recorded VERIFIED; External API Integration Layer still said generation was synchronous). Not yet committed (branch `rag-002-corpus`). **Prior session, retained below:** 🟢 **Built and verified CRITIC-001** (the vision-model critic/retry loop, FR-8), per direct team instruction to hold BACKEND-005/EVAL-004 and move to CRITIC-001 instead. Two open `TBD`s from the proposal got resolved by checking live rather than assuming (same practice ADR-019 used for Cloudflare's video catalog): (1) **vision-model provider** — Groq's live `/models` catalog has zero vision-capable models today, so the proposal's "likely the fallback LLM" guess doesn't hold; **Cloudflare Workers AI's `@cf/meta/llama-4-scout-17b-16e-instruct`** was picked instead (already-validated credentials from PROVIDER-001, no new signup), after a first candidate (`llama-3.2-11b-vision-instruct`) turned out gated behind a Community License requiring manual dashboard acceptance — skipped rather than accepted on the user's behalf. llama-4-scout was live-verified to return correctly-grounded structured verdicts against both a genuinely matching and a genuinely mismatching real image before any code was written against it. (2) **pathway scope** — discovered live during verification that Remotion's guaranteed pathway renders a stylized placeholder (a near-black theme background + a caption that fades in only after 50% duration — `remotion/src/theme.ts`/`compositions/*.tsx`), not a literal depiction of the shot description; running the critic against it correctly returned `fail` every time, which is the vision model working as intended, not a bug — but would exhaust retries on every Remotion shot for no benefit. **Scoped the critic to `pathway === 'external_api'` shots only** (Pollinations/Cloudflare images); Remotion shots skip the loop entirely. Recorded as **ADR-026**. Built `ai-service/critic/vision_client.py` (`evaluate_frame`, new `POST /critic/evaluate` FastAPI route) and `backend/src/services/criticService.js` (`runCriticLoop` — pathway filter, `ffmpeg` first-frame extraction for `.mp4`/direct read for images, the bounded regenerate-then-reevaluate loop), wired into `generationQueue.js` right after each shot reaches `'completed'`; reused the shot schema's pre-existing (previously unused) `retry_count` field rather than adding a duplicate counter, plus new `critic_passed`/`critic_reason` fields. A critic-loop infrastructure failure (ai-service unreachable, `ffmpeg` missing) is caught separately from generation failures — the shot stays `'completed'` with an explanatory `critic_reason` rather than being dragged down to `'failed'`. 7 new offline pytest tests pass (`ai-service/tests/test_critic.py`, `httpx.post` monkeypatched, mirroring `test_llm.py`'s pattern); full `ai-service` suite re-run afterward, **98/98**, zero regressions. **Live-verified end-to-end against the real stack** (real Express, real Bull/Redis, real MongoDB, real ai-service, real Cloudflare vision model, real Pollinations image generation, real Remotion render — no stubs) via temporary harnesses (`backend/_critic_verify*.js`, deleted after use): a real Pollinations "cozy library" image correctly passed critique with a grounded reason; a paired Remotion shot in the same storyboard correctly skipped the loop (`critic_passed` stayed `null`); and a deliberately adversarial description ("a single solid-black frame with... nothing else") produced two genuine critic fails, `retry_count` reaching exactly `CRITIC_MAX_RETRIES` (2), with the shot correctly finalized as `'completed'` using the last attempt's verdict rather than marked `'failed'` — a real, structured-feedback-triggered automatic retry cycle on the actual shipped code path, meeting FR-8's stated completion criterion. `CRITIC_MAX_RETRIES=2` was already documented as the confirmed default in `.env.example` (unused until now); added `CLOUDFLARE_ACCOUNT_ID`/`CLOUDFLARE_API_TOKEN`/`CRITIC_VISION_MODEL` there too, and ai-service now holds its own copy of the Cloudflare credentials (same real values as `backend/.env`) since it's a separate process, mirroring `GROQ_API_KEY`. CRITIC-001 marked **VERIFIED** in `PROJECT_ARCHITECTURE.md` Section 20 and here; completion moves to **30/36 (83.3%)**. Not yet committed (branch `rag-002-corpus`). Prior session in this run: **hardened R-14** (`backend/src/routes/storyboards.js`'s `POST /:id/generate`): two genuinely concurrent generate requests on the same storyboard used to have the loser hit a raw Mongoose `VersionError` (500) once both reached `doc.save()` in the "fresh run" write path. The fix does the fresh-run write as a compare-and-swap on `job_id` through the native MongoDB driver (`Storyboard.collection.findOneAndUpdate`) instead of Mongoose's `doc.save()`/`Model.findOneAndUpdate` — both of which turned out to add an implicit version-key guard on top of any update touching an array via `$[]`, which fights a hand-rolled CAS rather than composing with it (confirmed by first reproducing a `VersionError` through `Model.findOneAndUpdate` before switching to the native collection). The request that loses the CAS discards its now-redundant Bull job and hands back the winner's `job_id` instead of erroring. **Live-verified against the real stack** (real Express app, real Bull/Redis, real MongoDB — no stubs) via a temporary harness (`backend/_r14_verify*.js`, deleted after use, following the same "real services, script deleted after" pattern as ADR-023/BACKEND-006's verification): 5 genuinely concurrent `/generate` calls on one storyboard all returned `202` with the identical `jobId`, zero 500s, exactly one `job_id` persisted — repeated 3x, no flakiness. Separately confirmed no regression in the pre-existing sequential behavior: an immediate re-call while a job is still active still dedups to the same `jobId`, and calling `/generate` again after the job completes correctly starts a genuinely new job (regenerate). One incidental finding during verification, unrelated to the fix itself and already cleaned up: a stale `node` process from an earlier session was still bound to port 5000, causing the first two verification attempts to intermittently hit old code and report false failures — killed before the real verification runs. R-14 marked **Resolved** in `PROJECT_ARCHITECTURE.md` Section 24. R-14 is a risk-register item, not a Section 20 roadmap task, so completion stays **29/36 (80.6%)**, unchanged. Prior session: **built EVAL-003 (the multi-agent condition) and ran it live — 27/50 prompts succeeded for real before hitting Groq's actual per-model daily token cap. A scheduled auto-resume was set up, then cancelled per direct user instruction; EVAL-003 stays IN_PROGRESS, deliberately not being completed further right now.** `ai-service/evaluation/multiagent.py`'s `generate_multiagent(prompt)` calls the production `orchestrator.graph.build_graph()` directly with the raw prompt as `clarified_prompt` — no AI-003 clarification round-trip, mirroring Condition A's design (**ADR-025**, extending ADR-024) so the study compares "single-shot vs. multi-agent," not "clarified vs. unclarified" — and additionally surfaces `attempt_count`/`similarity_score` (present in the graph's final state, stripped by `generate_storyboard()`'s public shape) as retry/verdict diagnostics. `run_multiagent.py` mirrors `run_baseline.py`'s paced/isolate-failures/results-JSON shape, plus a new `--resume` flag that reuses already-`"ok"` records instead of re-spending tokens. 6 new offline pytest tests pass; the full suite came back **91/91**, zero regressions. **Live end-to-end in WSL2 against the real Groq API and the real RAG-003 index: 27/50 prompts succeeded — real RAG-grounded storyboards, every one a first-attempt pass (`attempt_count:1`), `similarity_score` 0.475–0.926, comfortably above the 0.35 threshold** — before the run hit a real `429`: Groq's per-model daily token budget for `llama-3.3-70b-versatile` turned out to be **~100,000 tokens/day in practice**, not the 1,000,000/day figure previously recorded (`memory/groq_free_tier_limits.md`, corrected this session), and the same-account fallback model was exhausted too, so there was no same-day recovery path. This is a genuine external quota wall, not a code defect — see **ADR-025**. Results (27 `"ok"` + 23 `"error"` with the real 429 message) persisted to `evaluation/results/multiagent_results.json`. A Windows Task Scheduler entry was set up to auto-run `--resume` once Groq's daily quota reset — the user then directed **cancelling that plan entirely**: the 23 failures have one understood, non-code cause and would very likely pass on retry, so waiting on/pursuing completion right now wasn't worth it (same pattern as BACKEND-005's video-tier hold, ADR-021). The scheduled task and its helper script were removed; `python -m evaluation.run_multiagent --resume` remains available to run manually later if the team wants the full 50. One genuine, reportable finding from the 27 completed prompts: the multi-agent condition's average spaCy `overall_score` dropped even more than EVAL-002's baseline (69.3 → 54.3, 26/27 scored lower after enhancement vs. EVAL-002's 28/50) — very likely because `run_multiagent.py` joins 3-5 short, fragment-like shot descriptions with `". "` for the comparison, which reads as choppier to the spaCy heuristic than Condition A's single flowing paragraph, not evidence of an actual quality gap; this reinforces, even more sharply than EVAL-002 did, exactly why ADR-006 designated spaCy as "reported, not ground truth" for this study. Completion stays at **29/36 (80.6%)** — EVAL-003 isn't VERIFIED, and per the team's decision isn't being actively pursued to 50/50 right now. Prior session: ran EVAL-002, the single-shot baseline condition, end-to-end against all 50 EVAL-001 prompts — EVAL-002 VERIFIED. `ai-service/evaluation/baseline.py`'s `generate_baseline(prompt)` is a single `complete_json` call with no clarification round-trip, no RAG grounding, no multi-shot decomposition, and no retry loop — the structural counterpoint to the AI-004/007/008 multi-agent pipeline (scope recorded as **ADR-024**). `ai-service/evaluation/run_baseline.py`'s `run(prompts)` drives every EVAL-001 prompt through it, paced at 2.5s/call to stay under Groq's free-tier 30 req/min cap, records both the original- and enhanced-text FR-1 spaCy scores per prompt (reported only — ADR-006 already rules spaCy out as the study's ground-truth metric), isolates any single prompt's failure into an `"error"`-status record instead of aborting the run, and writes one record per prompt to `evaluation/results/baseline_results.json`. 8 new offline pytest tests pass (mocked LLM + scorer, mirroring the AI-003/AI-004 monkeypatch pattern); the full `ai-service` suite was re-run afterward and came back **84/84**, zero regressions. **Live end-to-end in WSL2 against the real Groq API and the real spaCy pipeline: all 50 EVAL-001 prompts ran for real, 50/50 succeeded, zero errors.** Real media generation and the CLIPScore-style alignment metric were deliberately **not** run here — ADR-024 explains why (the rendering methodology isn't decided yet: which provider, which frame(s), video vs. still; rendering ~50-100 videos before EVAL-003/EVAL-004 settle that risks redoing them, so the decision is deferred to be made once for both conditions). One genuine, reportable finding surfaced by the real run, worth carrying into the final report as-is since FR-12's completion criterion explicitly accepts a directional result either way: the baseline's average spaCy `overall_score` did **not** improve (70.4 → 69.6 across the 50 prompts; 18 improved / 4 unchanged / 28 got worse) despite the enhanced text being visibly richer (~1045 chars average) — mostly because `temporal_coherence` (the ADR-010 finite-tense-mixing/temporal-marker heuristic) tends to drop when a single present-tense paragraph replaces a shorter original prompt. That's a property of the heuristic, not evidence the enhancement is worse — exactly the failure mode ADR-006 anticipated when it excluded spaCy as the study's ground truth; CLIPScore (EVAL-004) is the metric that will actually answer FR-12's question. Combined with EVAL-001, completion moves to **29/36 (80.6%)**. Prior session: first ran the real full-stack INTEG-001 confirmation (passed — INTEG-001 VERIFIED) and then curated the fixed 50-prompt evaluation set (EVAL-001 VERIFIED). EVAL-001: `ai-service/evaluation/dataset/` now holds 50 original prompts (`prompts.json`) across 15 genres, each tagged `single-beat` (26) or `multi-beat` (24) to explicitly satisfy the roadmap's "covering single-shot and multi-shot ideas" criterion, plus a strict validating loader (`loader.py`, mirroring RAG-002's `CorpusError` pattern via a new `EvaluationDatasetError`) and a README documenting the format and the fixed-set-stability rule (the set must stay exactly 50 and unchanged once EVAL-002/003 start using it, so both conditions see identical input). Prompts are deliberately moderate in descriptive detail — not maximally vague, not maximally detailed — so the upcoming baseline-vs-multi-agent comparison isn't decided by prompt quality alone. 9 new offline pytest tests pass; the full `ai-service` suite was re-run afterward and came back **76/76**, confirming no regressions (71 previously recorded + 9 new − 4 tests retired alongside the Producer agent, ADR-020, which happened after that 71-count was last recorded). INTEG-001 confirmation run (same session, done first): All four real services were started simultaneously (no stubs, no demo mode): `ai-service` via `uvicorn` in WSL2 (:8000), `backend` via `node src/app.js` (:5000), a real local MongoDB, real Redis in WSL2, and the frontend Vite dev server (:5173). Drove the actual browser UI through the full pipeline: typed a rich prompt → real `POST /api/prompts` (spaCy scored 86/100, no flags, clarify no-op) → clicked Generate → real `POST /api/storyboards` (the full LangGraph `screenwriter → cinematographer → intent_check` graph against the real Groq API and the real RAG-003 FAISS index — the resulting camera/style grounding used genuine corpus vocabulary: *silhouette, low-angle, high-contrast, volumetric, handheld, dolly-in*, not generic LLM filler) → real async `POST /:id/generate` (`202`) → real `GET /api/jobs/:id` polling from 0% to 100% → 4 real Remotion MP4s rendered and served at `/media`. Verified the videos weren't just "200 OK" but actually valid and playable: each `<video>` element's `readyState` was `4` (HAVE_ENOUGH_DATA), no `.error`, correct `1920×1080` + per-shot duration matching the storyboard's planned `duration_s`. Zero console errors for the whole session. **One real issue surfaced during the run, corrected after the fact:** `POST /api/prompts` and `POST /api/storyboards` each fired twice. First suspected as a `<StrictMode>` double-invoke frontend defect — **corrected same session** once the team clarified what actually happened: a team member had separately opened the same dev frontend and clicked Generate while this automated run was using the same dev backend, so two independent real clients collided, not one click producing duplicate events. The collision itself is still a real, worth-fixing finding: the two `POST /api/storyboards` calls created two storyboard docs, and their two `/:id/generate` calls raced — one lost with a Mongoose `VersionError` (500), the other succeeded, and the UI showed the correct end result with no visible error — self-healed, not blocking, but a real gap in the in-flight dedup's atomicity under genuine concurrent access. Filed and later corrected as **R-14** in `PROJECT_ARCHITECTURE.md` Section 24; the remaining work is just hardening the dedup, not further diagnosis. This closed the last explicitly-flagged item for INTEG-001. Combined with EVAL-001 above, completion moves to **28/36 (77.8%)**. Prior session (ADR-023 + frontend slice, 2026-08-11): wired the Bull/Redis queue for real (`POST /api/storyboards/:id/generate` → `202 {jobId}`, `GET /api/jobs/:id` polling, 11/11 HTTP integration checks) and wired the frontend to call/poll/render it (verified in demo mode). See git history / prior entries below for full detail. Uncommitted, branch `rag-002-corpus`. |
| **Last updated** | 2026-08-12 |
| **Last updated by** | Claude (Opus 5) — **FR-7 continuity injection + FR-9 assembly (ADR-027), from two team-reported defects.** Diagnosed both to one root cause (the pipeline's two halves were never connected: no `<Img>` in `remotion/src/`, and `world_state` accepted-but-unused on the API generation path). Built `backend/src/services/continuityPrompt.js`; widened `externalApiService`/`pollinationsImage`/`cloudflareImage` to carry seed/negative-prompt/reference options; started writing and reading `world_state.reference_image_url`; made the Remotion compositions image-first via `components/ShotLayer.tsx`; added `StoryboardVideo`/`render-storyboard.mjs`/`remotionService.renderStoryboard()` and wired assembly into `generationQueue.js` with `Storyboard.video_url`/`video_error`; extracted `remotion/src/shotTaxonomy.mjs`. Corrected two of my own design choices after looking at real generated output — shared seed → per-shot seed, and framing/action moved to the front of the prompt with motion words routed to Remotion instead. Live-verified with three real Pollinations generations + a real Remotion render (no stubs), fixing three real defects found en route (helmet CORP blocking `/media` for the headless browser — **restart a running backend to pick this up**; `render-shot.mjs` running `main()` at import; taxonomy unreachable from the browser bundle). Left explicitly unverified: the Cloudflare img2img tier. Updated `PROJECT_ARCHITECTURE.md` (FR-7, FR-9, §6.5, pipeline diagram, R-10 → Closed, new ADR-027), `PROJECT_STATE.yaml` (INTEG-002 → IN_PROGRESS; corrected two drifted component entries), and this file (§1/§3/§4/§8). Completion unchanged at 30/36 (83.3%). Not yet committed (branch `rag-002-corpus`). **Prior session, retained below:** Claude (Sonnet 5) — **CRITIC-001, vision-model critic loop (FR-8).** Per direct team instruction, held BACKEND-005 (no video-generation API access) and EVAL-004 (Groq daily quota exhausted, blocks on EVAL-003) and moved to CRITIC-001 instead. Resolved two proposal `TBD`s by checking live: (1) vision provider — Groq's live `/models` catalog has no vision-capable models, so picked **Cloudflare Workers AI's `@cf/meta/llama-4-scout-17b-16e-instruct`** (already-validated PROVIDER-001 credentials, no new signup) after a first candidate (`llama-3.2-11b-vision-instruct`) turned out gated behind a Community License needing manual dashboard acceptance — skipped rather than accepted on the user's behalf; live-verified against both a genuinely matching and mismatching real image before writing any code against it. (2) pathway scope — live verification revealed Remotion's guaranteed pathway renders a stylized placeholder (near-black theme background + late-fading text caption, not a literal scene depiction), so a literal vision-vs-text critic would fail every Remotion shot by design; **scoped the critic to `pathway === 'external_api'` shots only**, Remotion shots skip the loop. Recorded as **ADR-026**. Built `ai-service/critic/vision_client.py` (`evaluate_frame`, new `POST /critic/evaluate` route) and `backend/src/services/criticService.js` (`runCriticLoop` — pathway filter, `ffmpeg` frame extraction, bounded regenerate-then-reevaluate loop), wired into `generationQueue.js`; reused the shot schema's previously-unused `retry_count` field, added `critic_passed`/`critic_reason`. A critic-loop infrastructure failure is caught separately so it can't drag a successful generation down to `'failed'`. 7 new offline pytest tests pass (`ai-service/tests/test_critic.py`); full suite re-run, **98/98**, zero regressions. **Live-verified end-to-end against the real stack** (real Express, Bull/Redis, MongoDB, ai-service, Cloudflare vision model, Pollinations image gen, Remotion render — no stubs) via temporary harnesses (`backend/_critic_verify*.js`, deleted after use): a real Pollinations image correctly passed critique; a paired Remotion shot correctly skipped the loop; a deliberately adversarial description produced two genuine critic fails, `retry_count` reaching `CRITIC_MAX_RETRIES` (2), shot finalized `'completed'` with the last verdict — a real demonstrated automatic retry cycle on the shipped code path, meeting FR-8's completion criterion. Added `CLOUDFLARE_ACCOUNT_ID`/`CLOUDFLARE_API_TOKEN`/`CRITIC_VISION_MODEL` to `.env.example`'s AI-microservice section; ai-service now holds its own copy of the Cloudflare credentials (mirrors `GROQ_API_KEY`). Marked CRITIC-001 **VERIFIED** in this file and `PROJECT_STATE.yaml`; completion moves to **30/36 (83.3%)**. Updated `next_tasks` — with CRITIC-001 done and BACKEND-005/EVAL-004 deliberately held per instruction, next step needs a team call. Not yet committed (branch `rag-002-corpus`). Prior session, retained below: Claude (Sonnet 5) — **R-14 hardening.** Fixed the `POST /:id/generate` dedup race in `backend/src/routes/storyboards.js` (`PROJECT_ARCHITECTURE.md` Section 24): the "fresh run" write path used to load the storyboard, then call `doc.save()`; two genuinely concurrent requests could both pass the in-flight check, both reach `save()`, and the loser threw a raw `VersionError` (500) — the underlying gap behind the collision first observed during the INTEG-001 confirmation run. Replaced that write with a compare-and-swap on `job_id` via `Storyboard.collection.findOneAndUpdate` (the native MongoDB driver) — deliberately not Mongoose's own `findOneAndUpdate`/`save()`, both of which add an implicit version-key guard on top of any update touching an array via `$[]` that fights a hand-rolled CAS instead of composing with it (confirmed this the hard way: reproduced the same `VersionError` through `Model.findOneAndUpdate` first, before switching to the native collection). The request that loses the CAS now discards its redundant Bull job and returns the winner's `job_id` instead of a 500. Verified live against the real stack — real Express app, real Bull/Redis, real MongoDB, zero stubs — via a temporary harness (`backend/_r14_verify.js` + `backend/_r14_verify_regen.js`, deleted after use): 5 genuinely concurrent `/generate` calls on one storyboard all returned `202` with the identical `jobId`, no 500s, exactly one `job_id` persisted, repeated 3x with no flakiness; a second harness confirmed the pre-existing sequential paths are unaffected (in-flight dedup still returns the same `jobId` on an immediate re-call; a regenerate after completion correctly starts a new job). One incidental hiccup during verification, resolved and unrelated to the fix: a stale `node` process from an earlier session had port 5000, causing two early verification attempts to intermittently route to old code — killed before the runs that actually passed. Marked R-14 **Resolved** in `PROJECT_ARCHITECTURE.md` Section 24. R-14 is a risk-register item, not a Section 20 roadmap task, so the 29/36 (80.6%) completion count is unchanged. Updated `next_tasks` — with R-14 done and EVAL-003 a deliberate stop, nothing left is both unblocked and explicitly requested; flagged CRITIC-001 (technically unblocked via REMOTION-002) and EVAL-004/BACKEND-005's existing holds for a team decision rather than silently picking one. Not yet committed (branch `rag-002-corpus`). Prior session, retained below: Claude (Sonnet 5) — **EVAL-003, run the multi-agent condition (partial — quota-interrupted, then deliberately left incomplete per direct user instruction).** Wrote `ai-service/evaluation/multiagent.py` (`generate_multiagent(prompt)` — calls the production `orchestrator.graph.build_graph()` directly, raw prompt as `clarified_prompt`, no clarification round-trip per **ADR-025**, surfaces `attempt_count`/`similarity_score`) and `run_multiagent.py` (`run(prompts)` — same paced/isolate-failures/results-JSON shape as `run_baseline.py`, plus a `--resume` flag). New `ai-service/tests/test_evaluation_multiagent.py` (2 offline tests, mocking the same seams `test_orchestrator.py` mocks) and `test_evaluation_run_multiagent.py` (5 offline tests, including the new resume-reuse behavior) — 6 new tests, all pass; re-ran the full `ai-service` suite afterward and got **91/91**, zero regressions. Live end-to-end in WSL2 against the real Groq API + real RAG-003 index: ran `python -m evaluation.run_multiagent` over all 50 EVAL-001 prompts, paced 2.0s/prompt — **27/50 succeeded** (real, RAG-grounded, every one a first-attempt pass) before a real `429` from Groq's per-model daily token cap for `llama-3.3-70b-versatile` (~100,000 tokens/day in practice, discovered live this session — corrected `memory/groq_free_tier_limits.md`, which previously recorded 1,000,000/day) exhausted both the primary and the same-account fallback model. Added `--resume` (reuse already-`"ok"` records, only rerun the rest) specifically so the remaining 23 prompts can complete on a later day without re-spending tokens on the 27 that already succeeded. Wrote **ADR-025** (`PROJECT_ARCHITECTURE.md`) recording both the clarification-skip design decision (mirrors ADR-024's Condition A) and the quota-interruption/resume plan. Set up a Windows Task Scheduler entry to auto-run `--resume` once Groq's daily quota reset; the user then directed **cancelling that plan entirely** — the 23 failures have one understood, non-code cause and would very likely pass on retry, so pursuing completion right now isn't worth it (same pattern as BACKEND-005's video-tier hold, ADR-021). Removed the scheduled task and its helper script, amended ADR-025 with a same-day resolution note recording this decision. Marked EVAL-003 **IN_PROGRESS** (not VERIFIED, deliberately staying incomplete) in this file and `PROJECT_STATE.yaml`; completion percentage unchanged at 29/36 (80.6%). Updated `next_tasks`/§3/§10 to point at R-14 hardening as the actual next item. Prior session, retained below: Claude (Sonnet 5) — **EVAL-002, run the single-shot baseline condition.** Wrote `ai-service/evaluation/baseline.py` (`generate_baseline(prompt)` — one `complete_json` call, no clarification/RAG/decomposition/retry loop, per **ADR-024**) and `run_baseline.py` (`run(prompts)` — paces calls under Groq's free-tier cap, records original+enhanced FR-1 spaCy scores per prompt via `analyzer.score_prompt`, isolates per-prompt failures, writes `evaluation/results/baseline_results.json`). New `ai-service/tests/test_evaluation_baseline.py` (5 offline tests: single-call contract, malformed-description/camera rejection, whitespace trimming) and `test_evaluation_run_baseline.py` (3 offline tests: result-record shape, per-prompt failure isolation, one-record-per-prompt) — 8 new tests, all pass with the LLM/scorer monkeypatched; re-ran the full `ai-service` suite afterward and got **84/84**, zero regressions. Live end-to-end in WSL2 against the real Groq API + real spaCy pipeline: ran `python -m evaluation.run_baseline` over all 50 EVAL-001 prompts, paced 2.5s/call — **50/50 succeeded**, results persisted. Wrote **ADR-024** (`PROJECT_ARCHITECTURE.md`) recording the scope decision (text + spaCy scores now; real media generation and CLIPScore deliberately deferred to EVAL-003/004, since the rendering methodology isn't fixed yet and building ~50-100 renders before that risks redoing them) and the real finding from the run (baseline spaCy score didn't improve on average, 70.4→69.6 — a heuristic artifact per ADR-006/ADR-010, not evidence of a worse enhancement). Marked **EVAL-002 VERIFIED** in this file and `PROJECT_STATE.yaml`, updated the completion percentage to 29/36 (80.6%), and updated `next_tasks`/§10 to point at EVAL-003 (multi-agent condition — same runner shape, calls `orchestrator/graph.py` instead of `baseline.py`). Prior session, retained below: Claude (Sonnet 5) — **EVAL-001, curate the fixed 50-prompt evaluation set.** Wrote `ai-service/evaluation/dataset/prompts.json` (50 original prompts, 15 genres, tagged `single-beat`/`multi-beat`), `loader.py` (`load_prompts()`, strict validation, mirrors `rag/corpus/loader.py`'s pattern via a new `EvaluationDatasetError`), `__init__.py` files, and a `README.md` documenting the format, the moderate-detail design rationale, and the fixed-set-stability rule. New `ai-service/tests/test_evaluation_dataset.py` (9 offline tests: exact-50 count, unique ids/prompt text, both complexity values represented, >=10 diverse non-thin genres, tag/length minimums, missing-file error path) — all pass; re-ran the full `ai-service` suite afterward and got **76/76**, confirming zero regressions from the new code (the count itself dropped from the previously-recorded 71 to 67 first, due to 4 tests retired alongside the Producer agent in ADR-020, then +9 new = 76). Marked **EVAL-001 VERIFIED** in this file and `PROJECT_STATE.yaml`, updated the completion percentage to 28/36 (77.8%), and updated `next_tasks`/§10 to point at EVAL-002 (run baseline condition — now fully unblocked). Same session, done first — **INTEG-001 real full-stack E2E confirmation run.** Started all real services (ai-service/WSL2, backend, MongoDB, Redis/WSL2, frontend) and drove the live browser through Compose → Refine → Style → Generate with zero stubs/mocks anywhere. Confirmed the previously-isolated backend (HTTP integration test) and frontend (demo-mode run) halves of INTEG-001 genuinely connect end-to-end: real spaCy analysis, real Groq-backed LangGraph storyboard generation grounded in the real RAG-003 corpus, real async Bull job with live progress polling, real Remotion rendering, real MP4s confirmed playable in-browser (not just HTTP 200 — checked each `<video>` element's `readyState`/`duration`/dimensions/`.error` directly via JS). Zero console errors. Marked **INTEG-001 VERIFIED** in this file, `PROJECT_STATE.yaml`, and moved its Section 3 entry to Section 2. Surfaced **R-14** (initially misdiagnosed as a StrictMode frontend bug; corrected same session — the real cause was a team member concurrently using the same dev frontend while this run was in progress, causing two independent clients to race `/:id/generate` on the same storyboard, one hitting a Mongoose `VersionError` [500] that self-healed) — added to `PROJECT_ARCHITECTURE.md` Section 24's risk table, cause now known, hardening still open. Updated this file's §1/§2/§3/§4/§9/§10 and `PROJECT_STATE.yaml`'s top-level completion fields, `next_tasks` (→ EVAL-001), and the INTEG-001 task entry. Prior session (Opus 4.8) — **INTEG-001 frontend slice.** Wired the UI to the now-async generation backend (ADR-023): `handleGenerate` (`App.tsx`) chains storyboard creation straight into `POST /api/storyboards/:id/generate`, then polls `GET /api/jobs/:id` every 1.5s via a recursive `setTimeout` (`pollJob`), cancelled on completion/reset/unmount (`stopPolling` + a cleanup `useEffect`). `StoryboardView.tsx` gained a `GenerationBar` (progress %, state icon), per-shot `StatusPill`s (pending/processing/completed/failed/on_hold), and a `ShotAsset` renderer that shows `<img>` for stills or `<video>` for `.mp4` from each shot's `asset_url` (served at `/media`), merging live job data onto the structural storyboard shots by `shot_id`. A **Regenerate** button re-runs generation on the same storyboard. `lib/api.ts` gained `startGeneration()`/`getJob()` (+ a `getRequest` GET helper alongside the existing POST `request`), `lib/types.ts` gained `GenerateResponse`/`JobStatus`/`ShotStatus` + `Shot.asset_url`/`error`, and `lib/demo.ts` gained a **client-side simulation of the same poll loop** (`demoStartGeneration`/`demoGetJob`, timer-based per-shot completion with labeled SVG placeholders) so demo mode exercises the identical `App`/`StoryboardView` code path as the real backend. `tsc -b` + `oxlint` clean. **Live-verified in-browser via demo mode** (chosen because it drives the exact same polling/rendering code the real path uses, and the real `/generate`+`/jobs` endpoints were already verified 11/11 against the real stack in the prior session): drove the full flow twice — (1) default `remotion` provider: `Generation complete`, progress `0→100`, all 3 shots `Done`, 3 images rendered, `Regenerate`/`Create another` both present; (2) `huggingface` (video) provider: `Generation complete` at 100%, all 3 shots `On hold` with the amber *"pending a paid key"* message, **0** images rendered, matching the real backend's `HELD_PROVIDERS` behavior exactly. Zero console errors in both runs. Files: `frontend/src/App.tsx`, `frontend/src/components/StoryboardView.tsx`, `frontend/src/lib/{api,types,demo}.ts`. Updated this file's Current objective / Overall status / §3 / §10, and `PROJECT_STATE.yaml`'s `last_updated_by`, `next_tasks` (→ real full-stack confirmation run or EVAL-001), the INTEG-001 task note, and the Frontend Application component note. This closes the prompt→rendered-output loop in the **UI** for the image/Remotion pathways; the one thing not yet done is a **real** full-stack run connecting this UI to the real backend/ai-service/Mongo/Redis — see next_tasks. Prior session (ADR-023): Wrote **ADR-023** (`PROJECT_ARCHITECTURE.md`) for the async-generation design; updated Section 9.3 (`POST /storyboards/:id/generate` → async `202`; `GET /api/jobs/:id` PROPOSED→IMPLEMENTED), the Section 10.1 `jobs` row (deferred — Bull/Redis is the store), the Section 7 Queues status (→ IMPLEMENTED), the FR-6 dependency note, and the Section 20 INTEG-001 roadmap row; updated this file's Current objective / Overall status / §10, and `PROJECT_STATE.yaml`'s `last_updated_by`, `next_tasks`, the ADR-023 `decisions` entry, the INTEG-001 task (→ IN_PROGRESS), and the Backend API Gateway component note. Code: `backend/src/queues/generationQueue.js` (real per-storyboard worker), `backend/src/routes/storyboards.js` (async `/generate`, 202), `backend/src/routes/jobs.js` (new — `GET /api/jobs/:id`), `backend/src/models/Storyboard.js` (+`job_id`), `backend/src/app.js` (+jobs route), `.gitignore` (+`*.rdb`). Verified via a temporary HTTP integration harness (real app+Bull+Redis+Mongo, generation stubbed), 11/11, then deleted. Prior session (ADR-022): Wrote **ADR-022** (`PROJECT_ARCHITECTURE.md`) for the Web-Speech-API-over-hosted-Whisper decision; updated this file's Current objective / Overall status / §10 Next Recommended Actions, and `PROJECT_STATE.yaml`'s `last_updated_by`, `next_tasks` (voice-to-text → done; next = INTEG-001 async wiring), the ADR-022 `decisions` entry, and the Frontend Application component note. Code: `frontend/src/hooks/useSpeechRecognition.ts` (new — recognizer wrapper), `frontend/src/hooks/useDictation.ts` (new — press-and-hold gesture + field merge), `frontend/src/components/DictationMic.tsx` (new — reusable mic button), `frontend/src/components/PromptComposer.tsx` (composer mic + listening/error UI), `frontend/src/components/ClarificationChat.tsx` (per-answer mic). Two follow-up changes on the team's request in the same session: switched the interaction from click-to-toggle to true press-and-hold, and dropped the shared mic into the clarification chat. Prior: Claude (Sonnet 5) — ADR-021 (real per-shot image generation + single-image mode). Wrote **ADR-021** (`PROJECT_ARCHITECTURE.md`) covering both changes; updated FR-6's field table and added a new FR-6.1 subsection for single-image mode; updated Section 9.3 (`POST /api/storyboards/:id/generate` PROPOSED→IMPLEMENTED-with-deviation, added `POST /api/images`); updated the Section 20 BACKEND-005 roadmap row. Code: `backend/src/services/generationService.js` (new), `backend/src/routes/storyboards.js` (+`POST /:id/generate`), `backend/src/routes/images.js` (new), `backend/src/models/{Storyboard,Prompt}.js` (+`asset_url`/`error`/`on_hold` status; +`image_provider`/`image_url`), `backend/src/constants/renderProviders.js` (+`IMAGE_PROVIDERS`), `backend/src/app.js` (+`/media` static serving, +images route), `.gitignore` (+`backend/generated/`). Frontend: `lib/types.ts` (+`GenerationMode`/`ImageProvider`/`SingleImageResponse`), `lib/api.ts`/`lib/demo.ts` (+`generateSingleImage`/`demoSingleImage`), `StyleConfigurator/styleOptions.ts` (+`GENERATION_MODES`/`IMAGE_PROVIDER_IDS`/`StyleConfig.mode`), `StyleConfigurator/index.tsx` (+mode-toggle section, provider-list filtering, dynamic button label), `SingleImageView.tsx` (new), `FlowSteps.tsx` (+`resultLabel` prop), `App.tsx` (mode-based branching in `handleGenerate`), `vite.config.ts` (+`/media` proxy). Asked the user a scoping question first (verify-and-document only vs. actually wire real generation) before starting, since the two options differed by an order of magnitude in work and risk — team chose the larger scope. Prior: BACKEND-005 (external API adapter layer, image tier previously verified only at the direct-adapter-call level). |
...
```text
Task ID: INTEG-002
Description: Post-processing pipeline (FR-9), PROJECT_ARCHITECTURE.md Section 20 / Section 6.8.
  Scope changed this session: concatenation happens in Remotion, not via an FFmpeg concat
  demuxer (ADR-027). FFmpeg keeps the thumbnail/subtitle steps and FR-8's frame extraction.
Current state: LIVE-VERIFIED partial. remotion/src/compositions/StoryboardVideo.tsx (<Series>,
  one shot per duration_s) + remotion/render-storyboard.mjs assemble a whole storyboard into one
  MP4; remotionService.renderStoryboard() invokes it from generationQueue.js at the end of every
  generation job; result persists as Storyboard.video_url (failures as video_error).
  Verified 2026-08-12 with no stubs: 3 real Pollinations stills -> one 1920x1080 H.264 MP4,
  300 frames at 30fps (exactly the storyboard's 4+3+3s), ffprobe-confirmed. Frames sampled at
  1.5s/5.0s/8.5s show each shot's own still on screen with its camera move applied, and no text
  card anywhere. Shots without an asset_url are dropped rather than rendered as gaps, so a
  partially-failed storyboard still yields a watchable video.
  This also required making the Remotion compositions image-first (ADR-027) — they previously
  drew ONLY text and gradients, with no <Img> element anywhere in remotion/src/, which is why
  storyboards rendered through that pathway produced video of nothing but caption text.
Files: remotion/src/compositions/StoryboardVideo.tsx (new), remotion/render-storyboard.mjs (new),
  remotion/src/components/ShotLayer.tsx (new), remotion/src/shotTaxonomy.mjs (new — extracted so
  Node entry points and the browser bundle share one copy), remotion/src/{Root.tsx,types.ts,
  tsconfig.json,render-shot.mjs}, remotion/src/compositions/{Wide,Medium,CloseUp}Shot.tsx,
  backend/src/services/remotionService.js (+renderStoryboard), backend/src/queues/generationQueue.js,
  backend/src/models/Storyboard.js (+video_url/video_error), backend/src/routes/storyboards.js,
  backend/src/app.js (CORP fix)
Dependencies: REMOTION-002 (done), BACKEND-005 (image tier done; video tier held)
What remains: thumbnail frame extraction, subtitle overlay (both FFmpeg), and cloud media hosting
  (TBD provider — local disk only today).
Current blocker: none.
Next action: implement thumbnail extraction (simplest remaining piece — ffmpeg is already a
  verified dependency via criticService.js), then decide the cloud-hosting provider or explicitly
  scope it out for the FYP demo.
```
