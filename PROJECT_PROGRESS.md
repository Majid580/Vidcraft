...
| Field | Value |
|---|---|
| **Overall completion percentage** | **~72.2%** — computed as (verified tasks) / (total tasks in the Section 20 roadmap of `PROJECT_ARCHITECTURE.md`) = 26 / 36. Denominator moved from 35→36 and numerator from 25→26 this session: AI-005 (Producer/Router) is **retired** (ADR-020, no longer counted as a live deliverable either way), two new tasks were added to replace its function — **BACKEND-006** and **FRONTEND-004** — both now VERIFIED. BACKEND-005 remains IN PROGRESS (video tier still credit-blocked), not counted as verified. |
| **Current phase** | Phase 0/1 complete; Phase 4 (Remotion Integration) done; Phase 5 HIGH tasks (AI-002..006, minus retired AI-005) all complete; Phase 6 (RAG-001/002/003 + AI-007 + AI-008) fully complete; Phase 7's PROVIDER-001 decided, BACKEND-005 in progress (image tier done, video tier blocked), BACKEND-006/FRONTEND-004 (ADR-020's user-selected render provider) complete |
| **Current milestone** | M1 "Development environment and technology feasibility confirmed" — reached 2026-08-10 |
| **Current objective** | Two independent threads remain open: (1) finish BACKEND-005 — live-verify `generateByProvider('huggingface', ...)` once the account's monthly Inference Providers credits reset or the team pre-pays; (2) next feature, per direct team request: voice-to-text prompt input (a press-and-hold/press-to-talk button that streams transcribed speech into the prompt box), not yet started. Critical path for the pipeline itself is unchanged: BACKEND-005 → CRITIC-001 → INTEG-001. |
| **Overall status** | 🟡 **Significant architecture change this session (ADR-020): rendering pathway/provider is now an explicit user choice, not an agent decision.** The team's actual intent (clarified directly in chat) was never "let an agent silently decide when to spend real money" — it's "offer Remotion as a permanent free tier, plus named paid-but-cheap providers the user opts into deliberately, since this is a student FYP, not production software." Concretely: the Producer/Router agent (AI-005) is **retired** — removed from the LangGraph graph (`ai-service/orchestrator/graph.py` now 3 nodes: `screenwriter → cinematographer → intent_check → END`), its module deleted (`ai-service/orchestrator/agents/producer.py`, git history retains it), and the Screenwriter no longer emits a placeholder `pathway` field at all. In its place: the frontend Style Configurator gained a **Rendering method** section (4 tiles — Remotion/free, Pollinations/free, Cloudflare/free-tier, Hugging Face/~$0.05–$0.20 — by provider name, with a cost badge and a deliberate-choice disclaimer on paid picks), the user's single choice is sent as `renderProvider` in `POST /api/storyboards` and stamped onto every shot by the backend (`pathwayForProvider()`, `backend/src/constants/renderProviders.js`), and `externalApiService.js` was restructured from an auto-fallback abstraction into an explicit `generateByProvider(providerId, prompt)` dispatcher with **no cross-provider fallback** — once a user picks "Pollinations" by name, a failure surfaces as an error rather than silently trying Cloudflare instead. **All of this is live-verified**, not just written: ai-service pytest 67/67 (4 fewer than before — the retired producer-specific tests), a real `POST /api/storyboards` call with `renderProvider:"remotion"` produced a real 4-shot storyboard with every shot correctly carrying `pathway:"remotion", provider:"remotion"`, the same call with `renderProvider:"pollinations"` correctly produced `pathway:"external_api", provider:"pollinations"` on every shot, both validation error paths work (400 on missing/invalid `renderProvider`), the restructured `generateByProvider()` dispatcher was re-tested directly (real Pollinations JPEG; an unknown provider id throws `ExternalApiError` cleanly), and the full frontend flow was driven in a live browser (Compose → Refine → Style → real Groq/RAG-grounded 4-shot Storyboard) with zero console errors and a clean `tsc -b` + `vite build`. **BACKEND-005's own remaining gap is unchanged and separate from this**: the Hugging Face video tier is still blocked by depleted monthly credits (see prior session's finding) — that's an account/billing constraint, not touched by this session's architecture change. FR-1/FR-2/FR-3 remain implemented and verified end-to-end (FR-3's Producer/Router step is simply gone now, by design). Still open: no queue wiring yet for generation jobs (BACKEND-003's queue exists but has no real job type — that's INTEG-001); `POST /api/storyboards` is still synchronous (ADR-014); FR-7..FR-12 remain unimplemented; voice-to-text prompt input is a newly-requested, not-yet-started feature. Note: the project's Groq API key is free-tier (30 req/min, 1,000 req/day) — keep this in mind for any future live-LLM testing or the eventual evaluation study. |
| **Last updated** | 2026-08-11 |
| **Last updated by** | Claude (Sonnet 5) — implemented ADR-020 end-to-end (docs + code + live verification) in response to a direct correction from the team: the previously-built Producer/Router auto-routing (AI-005, ADR-013) was not what they wanted — they want explicit user control over cost, not an agent quietly choosing when to spend money, framed clearly as appropriate for a student FYP rather than production software. Asked 4 clarifying questions first (selection scope: once-per-video vs per-shot; AI-005's fate: removed vs kept-as-suggestion; option display: by provider name vs by tier; and whether to implement now vs docs-only) since getting the architecture decision wrong would have meant rewriting everything twice — team chose: once-per-video, remove AI-005 entirely, list by provider name, implement now. Wrote **ADR-020** and updated every `PROJECT_ARCHITECTURE.md` section that referenced Producer/Router (FR-3, FR-6, Sections 6.3/6.6/7/8/9/10/11.2/15, both architecture diagrams, both pipeline diagrams, the Section 20 roadmap — AI-005 marked retired, BACKEND-005/006 and FRONTEND-004 rows added/updated) plus marked ADR-013 superseded inline (not silently rewritten). Removed the Producer node from the ai-service LangGraph orchestrator and deleted `producer.py`; updated 67 ai-service tests accordingly (all pass). Added `backend/src/constants/renderProviders.js` as the single source of truth for the 4 valid providers; extended `Storyboard.js`'s schema (`render_provider` at storyboard level, `provider` per shot); extended `POST /api/storyboards` (BACKEND-006) to require + validate `renderProvider` and stamp it onto every shot; restructured `externalApiService.js` (BACKEND-005) into a no-fallback `generateByProvider()` dispatcher. Added the render-provider selector UI to the Style Configurator (FRONTEND-004) with cost badges and a paid-tier disclaimer; threaded `renderProvider` through `api.ts`/`demo.ts`/`types.ts`; updated `StoryboardView.tsx`'s shot badge to show the specific provider (not just a binary Remotion/External API split). Live-verified every layer: ai-service pytest, direct backend curl calls (both `remotion` and `pollinations`, plus both validation-error cases), the restructured dispatcher called directly, and a full real browser run through the actual running stack (backend + ai-service + MongoDB) with a real 4-shot storyboard and zero console errors. Nothing here touched BACKEND-005's separate, still-open Hugging Face credit blocker. Branch `rag-002-corpus`, not yet committed. Next up (per direct team request, not yet started): voice-to-text prompt input — a button that streams transcribed speech into the prompt box. Prior: BACKEND-005 (external API adapter layer, image tier verified). |
...
Current state: backend/src/services/externalApiService.js (generateImage/generateVideo) + backend/src/services/providers/{pollinationsImage,cloudflareImage,huggingfaceVideo}.js all exist.
  Image tier: LIVE-VERIFIED (both Pollinations and Cloudflare returned real JPEGs this session).
  Video tier: code-complete, auth/routing verified live, but the real generation call fails on a
  depleted Hugging Face monthly credit balance (InferenceClientProviderApiError).
Files: backend/src/services/externalApiService.js, backend/src/services/providers/*.js, backend/package.json (+@huggingface/inference)
Dependencies: PROVIDER-001 (done), BACKEND-003 (done)
What remains: live-generate one real video once Hugging Face credits reset or the team pre-pays — no code changes expected to be needed for that to succeed.
Current blocker: Hugging Face account has depleted its monthly Inference Providers free credits (discovered 2026-08-11). No Tier 2/3 fallback is wired up (fal.ai deliberately deferred, ADR-019), so there is currently no way to generate a real video through any connected provider until this resolves.
Next action: team decides whether to wait for the monthly reset (date unknown — check the HF account dashboard) or pre-pay credits (financial decision); once video is real-verified, move this row to Section 2 and mark BACKEND-005 VERIFIED.
```
