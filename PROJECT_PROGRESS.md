...
| Field | Value |
|---|---|
| **Overall completion percentage** | **~71.4%** — computed as (verified tasks) / (total tasks in the Section 20 roadmap of `PROJECT_ARCHITECTURE.md`) = 25 / 35 |
| **Current phase** | Phase 0/1 complete; Phase 4 (Remotion Integration) done; Phase 5 HIGH tasks (AI-002..006) all complete; Phase 6 (RAG-001/002/003 + AI-007 + AI-008) fully complete; Phase 7's PROVIDER-001 decided; into Phase 2/3/7 (Backend/DB, Frontend Dev, external-API adapter layer) |
| **Current milestone** | M1 "Development environment and technology feasibility confirmed" — reached 2026-08-10 |
| **Current objective** | BACKEND-005 (external API adapter layer) — the only remaining unblocked task now that PROVIDER-001 has selected concrete providers AND every Tier 1 credential is live-validated (ADR-019). Critical path: BACKEND-005 → CRITIC-001 → INTEG-001. No account setup remains: `CLOUDFLARE_ACCOUNT_ID`/`CLOUDFLARE_API_TOKEN` (image) and `HUGGINGFACE_API_TOKEN` (video — the actual provider, not a fallback) already work and live in `backend/.env`. |
| **Overall status** | 🟢 **The full FR-3/FR-4 orchestrator vertical (Screenwriter → Cinematographer → intent-similarity check → Producer/Router) is real end-to-end, and the external-API pathway now has concrete providers selected AND their credentials live-validated (PROVIDER-001, ADR-019) — only the adapter code (BACKEND-005) remains before an `external_api`-routed shot can actually render.** FR-1 (AI-002), FR-2 (AI-003), and FR-3 (AI-004/007/008/005) are implemented and verified end-to-end, and as of BACKEND-004 that whole chain round-trips through real REST routes (`POST /api/prompts`, `POST /api/prompts/:id/clarify`, `POST /api/storyboards`, `GET /api/storyboards/:id`) backed by MongoDB persistence and a real HTTP call to ai-service (`aiServiceClient.js`): a vague prompt (35/100, 3 flags) → 2 real Groq-generated clarifying questions → answers merged into a clarified prompt → a real LangGraph `StateGraph` (`screenwriter` → `cinematographer` → `intent_check` → `producer` → END) storyboard with RAG-grounded camera/style and an intent-similarity gate, persisted and fetchable. Error paths (400 validation, 404 unknown/malformed id, 502 ai-service failure) also verified. The remotion-routed shot pathway separately chains into REMOTION-003's `remotionService.renderShot()` (not yet wired into the storyboard routes themselves — that's INTEG-001). Still open: no queue (BACKEND-003) means `POST /api/storyboards` is synchronous rather than the originally proposed async 202/processing shape (ADR-014); shots routed to `external_api` still can't actually be rendered — the providers are now chosen (ADR-019) but BACKEND-005's adapter code doesn't exist yet. FR-6..FR-12 remain unimplemented. Note: the project's Groq API key is free-tier (30 req/min, 1,000 req/day) — keep this in mind for any future live-LLM testing or the eventual evaluation study. |
| **Last updated** | 2026-08-11 |
| **Last updated by** | Claude (Sonnet 5) — completed PROVIDER-001 (concrete Tier 1/2/3 external provider selection, ADR-019) in two passes. First pass: Tier 1 (free) = Pollinations.ai for image (`https://image.pollinations.ai/prompt/{text}`, no signup/key/account — live-verified via `curl`: 200 OK, real 512×512 JPEG with valid EXIF) and, based on secondary research, Cloudflare Workers AI for video. Once the user then supplied real Cloudflare + Hugging Face account credentials in chat, both were live-validated directly — and that check **overturned the video pick**: Cloudflare's real model catalog (queried live via `/ai/models/search`, filtered by task, by name substring, and by enumerating every task category across all 287 models in the account) has no video-generation model at all; the "FLUX 3 Video"/"HappyHorse 1.0" models the initial research cited don't exist there, most likely sourced from inaccurate secondary/SEO content. Corrected same-day, before any BACKEND-005 code was built on it: Hugging Face Inference Providers is now the actual Tier 1 **video** provider (not a fallback) — its token was live-verified via `GET /api/whoami-v2` (200 OK, correct `inference.serverless.write` scope) — and Cloudflare was kept as a validated **alternate image** provider instead of being discarded, since its real text-to-image catalog (FLUX.1/FLUX.2, SDXL, Leonardo Phoenix) and working credentials are still genuinely useful. Tier 2/3 (paid) = fal.ai selected as a single pay-as-you-go gateway but deliberately NOT wired into any code path yet, per the team's explicit instruction to stay free-only while testing. Updated `PROJECT_ARCHITECTURE.md` Sections 6.6/12/13, the R-8 risk row, and ADR-019 (twice — the correction is documented inline, not silently overwritten); updated `.env.example` and `backend/.env` with `CLOUDFLARE_ACCOUNT_ID`/`CLOUDFLARE_API_TOKEN`/`HUGGINGFACE_API_TOKEN`/`FAL_API_KEY` (real values in the gitignored `backend/.env` only, never echoed back in chat); fixed a stale `STORYBOARD_SIMILARITY_THRESHOLD=` (blank/TBD) to the actual ADR-018 default of `0.35` in both `.env.example` and `PROJECT_ARCHITECTURE.md` Section 13 while in those files. No BACKEND-005 code was written — this is the provider-selection decision plus credential validation, per this task's own defined deliverable ("Section 12 update") extended by the user directly supplying working credentials. Branch `rag-002-corpus`, not yet committed. Prior: AI-008 (intent-similarity retry loop). |
...
*(AI-008 and PROVIDER-001 moved to Section 2 — Verified.)*
