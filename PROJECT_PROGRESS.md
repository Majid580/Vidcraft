...
| Field | Value |
|---|---|
| **Overall completion percentage** | **~72.2%** — computed as (verified tasks) / (total tasks in the Section 20 roadmap of `PROJECT_ARCHITECTURE.md`) = 26 / 36 — **unchanged this session**: BACKEND-005 stays in Section 3 (In Progress), not Section 2, because its proposal-§5.2 acceptance criterion ("at least one real video generated") is still not met — that's an explicit, deliberate hold (ADR-021), not a completed task being under-counted. This session's real work (real per-shot generation wiring, single-image mode) strengthens BACKEND-005's *image*-tier evidence and adds a new capability outside the Section 20 roadmap's original task list, rather than closing a roadmap task outright. |
...
| **Current objective** | (1) INTEG-001's **backend async-generation slice is now done** (ADR-023): `POST /api/storyboards/:id/generate` is genuinely async (`202 {jobId}`), a Bull worker generates each shot with live per-shot Mongo progress, and `GET /api/jobs/:id` polls it — verified 11/11, resolving the ADR-014/ADR-021 synchronous stopgap. (2) **Next: the INTEG-001 frontend slice** — wire the UI to call `/generate`, poll `GET /api/jobs/:id`, and render the finished `asset_url`s (served at `/media`) in the storyboard view, closing the prompt→output loop for the image/Remotion pathways. EVAL-001 (fixed 50-prompt set) is fully unblocked and can go in parallel; making `POST /api/storyboards` async too remains open under INTEG-001. (3) BACKEND-005's video tier stays **deliberately held** pending a paid `HUGGINGFACE_API_TOKEN` (ADR-021). |
...
Current state: Backend async-generation slice DONE + verified (ADR-023, 2026-08-11). The BACKEND-003
  Bull/Redis queue now runs the real per-storyboard generation job:
  - POST /api/storyboards/:id/generate enqueues one job and returns 202 {jobId,storyboardId,status}
    (in-flight dedup via new Storyboard.job_id).
  - The in-process worker (backend/src/queues/generationQueue.js processGeneration) renders/generates
    each shot via the unchanged generationService adapters, persisting per-shot status/asset_url/error
    to Mongo incrementally and reporting job.progress() 0-100.
  - GET /api/jobs/:id (backend/src/routes/jobs.js) polls {jobId,state,progress,storyboardId,failedReason?,shots[]}.
  Verified via an HTTP integration run (real Express+Bull+Redis(WSL2)+Mongo, generation stubbed for
  speed/determinism): 202 -> active -> completed, progress 0/33/67/100, three shot outcomes
  (completed+asset_url / on_hold / failed+error), dedup + 404s -- 11/11, deterministic across 3 runs.
Files: backend/src/queues/generationQueue.js, backend/src/routes/storyboards.js,
  backend/src/routes/jobs.js (new), backend/src/models/Storyboard.js (+job_id), backend/src/app.js
Dependencies: AI-006, AI-008, REMOTION-003, BACKEND-005 (image tier), FRONTEND-002 -- all present
What remains: (1) FRONTEND slice -- after a storyboard is created, have the UI call POST /generate,
  poll GET /api/jobs/:id, show per-shot progress, and render the finished asset_urls (served at /media)
  in StoryboardView; (2) make POST /api/storyboards (creation) async too; (3) optional WebSocket progress
  push (polling is the fallback used now). Then a full prompt->delivered-output run closes the loop.
Current blocker: none (image/Remotion pathways). Video pathway stays a deliberate hold (BACKEND-005/ADR-021).
Next action: implement the frontend generate+poll+render slice against the now-async endpoints.
```
...
Task ID: INTEG-001 frontend slice
After a storyboard is created, wire the UI to POST /api/storyboards/:id/generate,
poll GET /api/jobs/:id, show per-shot progress, and render the finished
asset_urls (served at /media) in StoryboardView -- closing the prompt ->
delivered-output loop for the image/Remotion pathways (video stays held). Also
still open under INTEG-001: make POST /api/storyboards async too; optional
WebSocket progress push (polling is the fallback used now).
Dependencies:
Backend async endpoints (done, ADR-023) + FRONTEND-002/003 (done).
