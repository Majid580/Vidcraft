...
> **Source of truth for what exists:** the repository itself — always check the code, never this document, for what is built. *At this document's creation (2026-08-10) the repository contained one file, `VidCraft_Proposal.tex`, and this note read "pre-development / Day Zero". **That is no longer true and the note was stale for three days.*** As of 2026-08-13 the repository holds a working `frontend/`, `backend/`, `ai-service/` and `remotion/`, and the pipeline runs end-to-end: a prompt is scored, optionally clarified, decomposed into shots by a LangGraph agent graph, grounded against a 75-passage RAG index, checked for intent, rendered per shot with a vision critic that regenerates failures, assembled into a single 1080p MP4, and post-processed into a poster, captions and a hardsubbed copy. **32 of 36 roadmap tasks are VERIFIED (88.9%)** and the live demonstration has been rehearsed three times against the real stack. What remains is the evaluation study (EVAL-003/EVAL-004, held on Groq quota) and the final report. **See [`PROJECT_PROGRESS.md`](PROJECT_PROGRESS.md) Section 5 for the authoritative list of what is verified — not the per-capability status tags in this file.** Those tags have repeatedly gone stale: on 2026-08-13 the Prompt Analyzer, the Clarification Agent, the Frontend and the Backend API Gateway were all still tagged `NOT_IMPLEMENTED` here while being VERIFIED, in daily use, and demonstrated live. They were corrected, but the lesson is structural — this document is edited when the *design* changes, and implementation status is a different clock. Trust the progress file.
> **Last updated:** 2026-08-13
> **Last updated by:** Claude (Opus 5), documentation-sync pass — corrected this header (it still described a Day Zero repository and still claimed `NOT_IMPLEMENTED` applied to 100% of the system), alongside a reconciliation of `PROJECT_PROGRESS.md` and `PROJECT_STATE.yaml`, whose phase rollups had never been maintained. No architectural change; the ADR log through ADR-029 is unaffected. Prior substantive revision: Claude (Sonnet 5), 2026-08-11, syncing AI-008 (sentence-similarity intent check + retry loop) into Sections 6.3, 20 (risk table), 24 and the ADR log, in direct collaboration with the project lead (Muhammad Majid).
> **Document status:** Live. Authoritative for intended design until superseded by a later dated revision of this file.
...
| `NOT_IMPLEMENTED` | Zero corresponding code exists in the repository. *(This row read "as of this writing, this applies to **100% of the system**" until 2026-08-13, which had been false since the first code landed on 2026-08-10. It now applies only to the specific capabilities still carrying the tag — chiefly the video-generation tier and cloud media hosting. `PROJECT_PROGRESS.md` Section 5 is the authoritative list of what is verified.)* |
...
React Frontend  [VERIFIED — FRONTEND-001..004]
  ↓ REST (prompt submission, storyboard review) + polling on GET /api/jobs/:id
  ↓ (WebSocket/Socket.IO progress push remains PLANNED — polling is what ships, INTEG-001)
Node.js / Express Backend  [VERIFIED — BACKEND-001..004, 006]
  ├─ API Gateway (routes, validation, no auth per R-9)
  ├─ Bull.js Job Queue (backed by Redis) — async generation jobs  [VERIFIED — INTEG-001]
  └─ Socket.IO Server — real-time status push to frontend  [PLANNED — frontend polls instead]
  ↓ internal HTTP call
Python FastAPI AI Microservice  [VERIFIED — AI-001..008, RAG-001..003]
...
Post-Processing — FR-9  [VERIFIED — INTEG-002]
  ...
  ↓
Storage: MongoDB (prompts/storyboards)  [VERIFIED — BACKEND-002/003]
         + Cloud media hosting  [NOT_IMPLEMENTED — provider TBD; media is served from local disk via /media]
         (embeddings live in the FAISS sidecar, not Mongo — ADR-017)
  ↓ WebSocket push
...
- **Current implementation status:** `VERIFIED` (AI-002). `ai-service/analyzer/` — `score_prompt()` returns the five FR-1 dimensions and a 0–100 overall. In continuous use well beyond its own task: it is what EVAL-002/003 score both study conditions with, and what the UI's analysis panel renders. *(This line read `NOT_IMPLEMENTED` until 2026-08-13, long after the component was verified and depended upon.)*
...
- **Current implementation status:** `VERIFIED` (AI-003). `ai-service/clarification/` — question generation plus a resolve step that merges answers into a clarified prompt, exercised end-to-end through the UI's clarification chat (FRONTEND-002) and rehearsed live in DEMO-001. *(This line read `NOT_IMPLEMENTED` until 2026-08-13.)*
...
- **Current implementation status:** `VERIFIED` (FRONTEND-001/002/003/004). A guided Compose → Refine → Style → Storyboard SPA that drives the whole pipeline in the browser, including generation polling and the assembled video with poster, captions and downloads. Visual identity re-grounded per ADR-029. *(This line read `NOT_IMPLEMENTED` until 2026-08-13.)*

### 6.10 Backend API Gateway

- See Section 8 in full.
- **Current implementation status:** `VERIFIED` (BACKEND-001..004, 006). Express app with the real REST surface, Bull job queue on Redis, and MongoDB persistence. *(This line read `NOT_IMPLEMENTED` until 2026-08-13.)*
...
### ADR-030: EVAL-004 scoring design — stills over video, one fixed reference text, and both aggregations reported

- **Date:** 2026-08-13
- **Context:** ADR-024 and ADR-025 deliberately deferred two things out of EVAL-002/EVAL-003 rather than guess: the "Generate output - same backend" step in Section 4.7's diagram, and the CLIPScore-style metric it feeds. EVAL-004 has to settle both. It was picked up because it is the only remaining roadmap task that does **not** depend on the paid video tier — the question that prompted this session was precisely "what can be done without a paid video-generation API", and the answer is: all of the evaluation study, because CLIP scores frames.
- **Selected:** Four decisions. **(1) Stills, not video.** The metric is frame-level by construction — CLIP embeds an image, not a clip — so a still is what it consumes either way. BACKEND-005's video tier is held on a paid key, and the Remotion pathway animates *these same stills* with a deterministic Ken-Burns move rather than synthesising new pixels, so for both available pathways the still IS the generated content. **(2) Pollinations, not Cloudflare.** Cloudflare FLUX is the more reliable Tier 1 provider (DEMO-001: 4/4 vs 2/4) and was the first choice; its free allocation of 10,000 neurons/day was already exhausted, so Pollinations is the only provider available at this scale without payment. **(3) Every frame is scored against the ORIGINAL prompt** — never against the text the condition itself produced. This is the load-bearing decision, not the model choice. Scoring a frame against its own generating text would measure how faithfully Pollinations followed whatever it was handed, which is a property of the image model rather than of the pipeline under test, and it would reward verbosity in the enhancement step. Holding the reference identical across conditions is what makes this controlled. The image seed is likewise fixed per prompt and shared across conditions, so seed luck cannot favour an arm, and the set is **paired** — only prompts that succeeded under both conditions are compared. **(4) Both aggregations of Condition B are reported** — mean-of-shots and best-shot — never one alone.
- **Why:** Decision (4) exists because a naive reading of this metric would defame the multi-agent pipeline. Condition B decomposes a prompt into shots that each depict a *part* of the scene: an establishing wide, then a close-up of one detail. Condition A produces a single frame trying to summarise the whole prompt. Scored against the full original prompt, B's individual frames are therefore expected to score lower **by construction** — a close-up of a hand on a collar genuinely does not depict "a rain-soaked neon alley at night" as completely as one wide summary frame does. That is storyboarding working as intended, and a mean-of-shots figure published on its own would misrepresent it as a defect. Reporting the best-shot figure alongside it asks the different and fairer question: does the storyboard contain a frame that covers the prompt at least as well as the baseline's single frame? The deeper limitation is stated in the generated table itself and must survive into the final report: **CLIPScore reads one frame at a time and has no notion of a sequence, so it is structurally blind to what Condition B is actually for** — narrative decomposition, shot variety and cross-shot continuity (FR-7). A study reporting only this number would be measuring the baseline's strongest axis and none of the multi-agent pipeline's. Separately, the render prompt is composed identically for both conditions (`{camera}. {description}.`) and does **not** reproduce `continuityPrompt.js`'s production composition, which additionally prepends Condition B's `world_state` as a labelled Subject/Location/Style block: maintaining a Python copy of that JS is exactly the drift ADR-028 refused when it exported `assembledShots()` instead of duplicating the shot filter. Note the direction of that bias — `world_state` is a genuine product of the multi-agent pipeline with no Condition A equivalent, so excluding it *understates* Condition B, making any measured B win a floor rather than a ceiling.
- **Consequences:** Three new modules under `ai-service/evaluation/`: `media.py` (frame generation, disk-cached by content hash, rate-paced), `alignment.py` (CLIPScore = `2.5·max(cos,0)` on `clip-ViT-B-32` via `sentence-transformers`), and `run_scoring.py` (the runner, emitting `results/eval004_scores.json` and the `results/eval004_comparison.md` deliverable Section 4.7 calls for). Running CLIP locally is the narrow exception ADR-015 already granted to `sentence-transformers`+`torch`, not a new departure: it is a one-off scoring pass over a few hundred images that never touches the request path. `pillow` added to `requirements.txt`. **38/38 offline edge checks passed** via a temporary harness, deleted after use per project convention; two real defects were found by writing them — the minimum-size guard sat inside the Pollinations fetch, so any other fetch implementation could cache a non-image (a cached error page is indistinguishable from a real frame on the next run and would silently corrupt the scored set), and `spacy_summary` indexed record keys directly instead of tolerating absent ones. **Generated frames are committed**, unlike the FAISS index which `.gitignore`s as a derived artifact: the index rebuilds deterministically from a committed corpus, whereas a hosted diffusion provider offers no such guarantee, so these frames are the study's only durable evidence. **Provider pacing was measured, not assumed** — unpaced ≈100s/frame (nearly all retry backoff), 5s spacing ≈50s/frame still 429ing, 15s spacing 0 rate-limits across 6 consecutive requests; the residual 1.9–31.8s per request is the provider's own queue and no amount of pacing or concurrency removes it.
- **Status — a working scoring arm and a PILOT result, not the study.** The machinery is verified end-to-end on real data: real Pollinations frames, real CLIP, zero retries, zero failed frames. But the provider's queue settled at roughly one frame per minute, putting the full 222-frame set at ~3 hours of wall-clock, and the team elected to stop generation and take the table from what was cached. **The published table is therefore n = 5 of the 46 paired prompts available**, and it says so in a banner at the top of its own first page rather than in a footnote. At that n nothing is established: mean-of-shots came out −0.017 (p = 0.685) and best-shot +0.078 (p = 0.136), neither remotely significant, and the per-genre breakdown is 2–3 prompts per cell. **Do not quote these numbers as findings.** Two things nonetheless carry: the *method* is now fixed and reviewable, and the run costs nothing to finish — every frame is cached by content hash, so re-running `python -m evaluation.run_scoring` resumes rather than restarts. One guard exists specifically because of this stopping mode: a prompt is admitted only when **every** one of its frames is cached, because a mean-of-shots averaged over whichever shots happened to finish first would be a different statistic wearing the same name — 41 partially-generated prompts were excluded on that rule rather than quietly averaged.

### ADR-031: Clarification question count scales with how under-specified the prompt is

- **Date:** 2026-08-13
- **Context:** Observed in use: the clarification step asks **two questions every time**, whether the prompt is "a dog" or a fully-formed scene description. `MAX_QUESTIONS = 2` was a hard constant and `generate_questions` ended with `questions[:MAX_QUESTIONS]`. That slice was silently discarding the analyzer's own evidence — FR-1 emits one flag per dimension scoring below threshold, so "a dog" produced **four** flags naming four distinct gaps, and two of them were thrown away before the user ever saw them. The reverse failure was just as real: a prompt with one genuine gap still got two questions, the second necessarily padding.
- **Selected:** The budget is computed from the analysis rather than fixed. **One question per distinct flag** — a flag *is* an unresolved gap, and asking past the gaps produces filler; flags are de-duplicated first because `contradictory_descriptors` is appended once per contradicting pair. **Plus one if the prompt is shorter than `CLARIFICATION_SHORT_PROMPT_WORDS` (12)**, because the analyzer can only flag what was written and cannot flag detail that was never expressed at all. **Plus one if the overall score is below `CLARIFICATION_LOW_SCORE` (40)**, which means weak everywhere rather than weak in one place. Clamped to `CLARIFICATION_MAX_QUESTIONS` (6). Zero flags still means zero questions and **no LLM call at all**. The FR-1 `overall_score` is now threaded from the backend through `/clarify/questions` as an **optional** field, so a caller that sends only flags keeps working.
- **Why:** The prompt-side change matters as much as the count. The old instruction said "ask **at most** 2" — a ceiling, which a model satisfies by asking two generic questions. It now says "ask **EXACTLY** N", with the reasoning that the number was already derived from this prompt's real gaps, plus explicit quality rules: one gap per question and never two questions one answer would settle; only things the camera can show (not intent, audience or purpose); **never yes/no**, because "Is it at night?" gains one bit where "What time of day, and what is the main light source?" gains a usable answer; and two or three concrete example directions in parentheses, because a user who wrote a vague prompt frequently does not yet know what the options are. `build_brief`'s instruction was tightened in the same pass: the clarified prompt is the only thing the rest of the pipeline ever reads, so it must carry every answer, and it may add specificity *implied* by the answers (light from a stated time of day, texture from a stated location) but must **not** invent characters, plot, places or dialogue — inventing there is worse than leaving a gap, because everything downstream treats it as the user's intent. **This stays a single round**, which is what guarantees termination (proposal §6.2): a vaguer prompt now gets a *longer* round, never a second one.
- **Consequences:** `ai-service/clarification/agent.py` gains `question_budget()`; three tunables added to `config.py`; `ClarifyQuestionsRequest.overall_score` added in `main.py`; `aiServiceClient.getClarificationQuestions()` and the `POST /api/prompts` route pass it through. **No frontend or API-shape change was needed** — `questions` is still `string[]` and `ClarificationChat.tsx` already rendered whatever length it was handed. **110/110 tests pass** (19 new). Measured against real spaCy: `"a dog"` (score 15, 4 flags) 2 → **6** questions, `"a man walks"` (score 35, 3 flags) 2 → **5**, while an 83-scoring rich prompt stays at **0** and still skips the step entirely. Verified live against the real Groq API: `"a dog"` now returns six distinct, non-overlapping questions covering action, location, breed/appearance, time-of-day and light, mood, and other subjects — each with worked examples — where the old build asked two generic ones. **Cost note:** a maximally vague prompt now costs one LLM call returning ~6 questions instead of ~2; the call count is unchanged, only the output length, so the Groq daily-token exposure barely moves.

### ADR-032: Narration as measured beats — the video is fitted to the audio, not the reverse

- **Date:** 2026-08-13
- **Context:** Two linked questions from the team: should a video get "a full script — what happens 0-1s, 1-2s, and so on", and is voiceover already produced by video-generation models or must it be added afterwards? The second was settled by checking rather than assuming. **Every model in this project's reach is silent** — Runway, Pika, Luma, Kling, Stable Video Diffusion and the Hugging Face text-to-video models all return picture only; native synchronised audio exists in Veo 3 and Sora 2, neither of which is in PROVIDER-001's tiers. So voiceover must be added after generation, which immediately raises the two hard questions: what should the audio *say*, and how does it stay in sync?
- **Selected — beats, not seconds.** A fixed one-second grid fails twice over. **Nothing can consume it:** the Remotion pathway holds one still per shot under a Ken-Burns move, image providers return one still per shot, and text-to-video models take one prompt per clip rather than instructions addressed to `t=3s` (keyframes are the finest control that exists anywhere). And **the unit is wrong**: a 4-second shot holds about two narrative beats, not four, so an equal-slice grid invents three boundaries that do not exist and forces padding. A **beat** — one continuous thing happening, with one line of narration over it — is the smallest unit that is real. A per-second table is a *view* renderable from beats; the reverse derivation is not sound.
- **Selected — what the audio says.** Narration is **never** inferred from generated pixels. It is written by a Narrator agent (`ai-service/narration/agent.py`) from the storyboard the pipeline already has, so picture and voice share one source. This is the same shape as ADR-028's subtitles, which are likewise text timed to the assembled shot list rather than anything read back off the video.
- **Selected — how it stays in sync: the video is fitted to the audio.** The tempting direction is to constrain narration to a shot's authored `duration_s` at some words-per-second rate. That drifts, because speaking rate varies with voice, punctuation and words, and the error accumulates until the voice describes the wrong picture. Instead each line is synthesised **first**, its real spoken length **measured**, and the shot's `duration_s` set to the sum of its beats. Remotion composes an *authored* timeline — `framesFor(shot) = round(duration_s × FPS)` takes duration as an **input** — so the shot becomes exactly as long as the speech it carries and **drift is not reduced, it is structurally absent**. Notably this required **no timing change in the composition at all**: `framesFor` already read `duration_s`, so fitting video to audio was a matter of feeding it measured numbers.
- **Why the measurement is trustworthy:** Edge TTS streams `SentenceBoundary`/`WordBoundary` metadata alongside the audio carrying offset and duration in 100-nanosecond ticks, so the spoken length is *reported by the synthesiser* — no MP3 decoding, no `ffprobe`, no estimate. When a provider reports a duration under `MIN_SPOKEN_SECONDS` the synthesis **raises rather than guessing**, because a fallback estimate there would reintroduce precisely the drift this layer exists to remove.
- **Provider (checked live 2026-08-13, all three):** **Edge TTS is Tier 1** — hosted (so ADR-015's no-local-inference rule holds: synthesis runs on Microsoft's servers, nothing loads onto the dev machine), free, and requiring no key or signup. **Cloudflare Workers AI is Tier 2** — `@cf/myshell-ai/melotts` and `@cf/deepgram/aura-1` both confirmed to exist (they return schema-validation errors, not 404s) but share one 10,000 neuron/day allocation with image generation, which EVAL-004's frames had already exhausted. **Groq is out entirely:** `playai-tts` returns *"has been decommissioned"*, and its live model list now holds only Whisper — which is speech-to-**text**.
- **Consequences:** New `ai-service/narration/` package (`agent.py`, `tts.py`) and `POST /narration/script`, which writes the script, speaks it, and returns beats carrying measured durations plus base64 audio, with the shot durations the video must adopt. Per-beat synthesis failure is **non-fatal** — that beat becomes a silent slot of the same shape, so one dead line costs its voiceover and not the whole render. Silent beats are a legitimate authoring choice and still occupy `SILENT_BEAT_SECONDS`. `remotion/src/types.ts` gains `Beat`; `StoryboardVideo.tsx` gains a `BeatAudio` layer placing each clip at its beat's offset. `edge-tts==7.2.8` added to `requirements.txt`. **18 new tests, all passing**, including that the timeline is contiguous with no gaps or overlaps, that a shot's duration equals the sum of its beats, that total timeline equals total measured speech, and that an unmeasured or zero-duration beat **raises** rather than being timed by guesswork. Verified with real synthesis end-to-end through the real endpoint: three real lines measured 3.212s / 2.487s / 2.462s, laid onto a contiguous timeline totalling 8.162s against 8.162s of measured speech. **Backend wiring, completed the same day.** `narrationService.attachNarration()` runs in `generationQueue` **after** per-shot generation and **before** assembly, and both halves of that ordering are load-bearing: after, because narration is written only for shots that actually survived — a shot whose provider failed is dropped from the video and occupies no time, so narrating it would push every later line ahead of its picture (ADR-028's guard, and not hypothetical: real provider 500s drop shots on ordinary runs); before, because it *replaces* each narrated shot's `duration_s` with the measured sum of its beats. Audio is written to `/media/{id}/narration-{shot}-{beat}.mp3`, beats persist on the shot subdocument (so `GET /api/jobs/:id` returns them with `shots` for free), and `remotionService` maps them across explicitly — that mapping is a field-by-field construction rather than a spread, so beats had to be carried deliberately and their URLs given the same relative→absolute treatment as the stills. Failure is non-fatal at every level, like assembly (ADR-027) and post-processing (ADR-028): shots keep their authored durations and the video renders silently, with `narration_error` explaining why. **Captions now follow beat boundaries** when a shot is narrated — a caption saying something other than the voiceover would be worse than none.
- **A real rounding defect, found and fixed while wiring it.** Rounding each beat's duration to frames *independently* lets a shot's beats sum to a different frame count than the shot itself: two 1.11s beats round to 33+33=66 frames while their 2.22s shot rounds to 67. One frame is enough to push an `<Audio>` clip past the end of its `<Series.Sequence>`, to desynchronise captions from speech, and — worst — to corrupt the frame total that ADR-028's desync guard checks against Remotion's actual render, silently disabling the very check that exists to catch this class of bug. Both consumers now derive boundaries by **cumulative** rounding within the shot, clamped to the shot's own frame count, so beats sum to their shot exactly and by construction. `ffmpegService.buildVtt` and `StoryboardVideo.tsx`'s `BeatAudio` must round identically or the voice and subtitles describe different moments; that requirement is stated in both files. Verified: 10/10 checks on the caption builder (including that independent rounding genuinely *would* have disagreed, and that total frames are unchanged with narration on or off, so the guard still holds) and 14/14 on the narration service (including that un-generated shots are never narrated and that failure leaves authored durations untouched).
- **Live-verified end to end with sound, 15/15 — and notably with none of the exhausted services.** The first real render carrying a voiceover was produced using only free/local parts: Edge TTS for the speech, the EVAL-004 frame cache for the stills, local Remotion, local FFmpeg, and a minimal stand-in for the `/media` mount. **No Groq, no Cloudflare, no video-generation API** — which is the point, because all three were exhausted or unavailable when it ran. Result: a 14.144s H.264 file against a 14.075s measured timeline, **423 frames exactly as the composition reported**, and an **AAC audio stream present**. The synchronisation claim was *measured rather than asserted*: the audio track was extracted and its RMS sampled in the middle of every beat, giving ~1886–2348 in each of the four spoken beats and **0 in the deliberately silent one** — speech lands precisely where the beat timeline says it should, and nowhere else. Captions agreed independently: four cues for four spoken beats, carrying the narration rather than the shot description, starting at `00:00:00.000`, with the silent beat correctly appearing as a **gap** (cue 3 ends 10.033s, cue 4 resumes 11.533s) while still consuming its time. `postProcess` reported no error and produced poster, subtitles and hardsub as usual. Temporary harness deleted after use, per project convention. **What this run did NOT exercise:** the Narrator agent itself (it needs Groq, whose daily tokens were spent) — the narration text was hand-authored for the harness, so the LLM authoring step remains covered only by its unit tests, while everything downstream of it is now proven on real output.

### ADR-033: Cloudflare request bodies are built per model — flux-1-schnell never accepted the continuity parameters

- **Date:** 2026-08-14
- **Context:** Reported from a real 4-shot run: two shots rendered, two failed with `Provider 'cloudflare' failed: AiError: Bad input: Error: Additional or unevaluated properties '/seed, /negative_prompt' at '/' not allowed` (code 5006). Workers AI validates every request body against **that model's own published input schema** (`GET /accounts/{id}/ai/models/schema?model=…`) and rejects undeclared properties. The schemas are **not uniform across the catalog**, but `providers/cloudflareImage.js` built one body shape for the whole provider. `@cf/black-forest-labs/flux-1-schnell` — the default since BACKEND-005 — declares exactly two properties, `prompt` and `steps`. It had never accepted this project's FR-7 continuity parameters at all.
- **Why it took a live storyboard to surface it.** Two things hid it. **(1) Enforcement is inconsistent across Cloudflare's fleet.** Probed directly: the *same* illegal body returned `200` once and `400` twice in three consecutive calls, which is why a single run failed on shots 1 and 3 and rendered shots 2 and 4 — the body was identical for all four, so nothing about the shots explains it. **(2) The requests that slipped through were silently wrong.** flux has no `seed` and no `negative_prompt`, so it discarded both: every Cloudflare render ever produced by this project was unseeded and unguarded, while the adapter's return value reported a `seed` that had been dropped. `continuityPrompt.js`'s claim that "the same (storyboard, shot) always regenerates the same image" was true for Pollinations and **false for Cloudflare**, and `DEFAULT_NEGATIVE_PROMPT`'s anti-collage/anti-watermark/anti-identity-drift guard was a no-op there.
- **Selected — a per-model parameter table, not a per-provider one.** `MODEL_INPUTS` lists each model's declared schema properties and `buildBody()` emits only those. An unlisted model is sent `prompt` alone — the one property every text-to-image schema in the catalog declares — so an unrecognised model degrades to a plain render instead of a 400. Verified mechanically against the live schema endpoint: every parameter in the table exists in the model it is listed under. Registering a model is now a precondition for using it, which is what makes this class of bug structurally unable to return.
- **Selected — `@cf/leonardo/phoenix-1.0` as the default text-to-image model.** Three schema-compatible candidates were measured live on one prompt, 1024×1024, with seed and the full negative prompt (2026-08-14):

  | model | latency | same seed twice | output |
  |---|---|---|---|
  | **leonardo/phoenix-1.0** | **~3.1s** | **identical bytes** | photoreal; `content-type` matches its bytes |
  | bytedance/sdxl-lightning | ~4.0s | differs | plastic-looking; labels JPEG bytes `image/png` |
  | stabilityai/sdxl-base-1.0 | ~10.8s | differs | slowest; 1.2 MB PNG |

  **The determinism column decided it, not the speed column.** FR-12's evaluation and `seedForShot` both rest on a fixed seed reproducing a fixed image; Phoenix is the only one of the three where that actually holds — and it is also the fastest and the only one whose `content-type` header matches its own bytes, which matters because that header names the file on disk. flux-1-schnell stays in the table so an explicit `options.model` still works; it is no longer reachable by default.
- **A second, independent defect the fix exposed.** With a model that finally *reads* the seed, Phoenix rejected roughly half of them: `'/seed' must be >= 0`. `seedForShot` mixed with `Math.imul`, which returns a **signed** 32-bit result, and JavaScript's `%` preserves the sign — so the function had been emitting negative seeds since FR-7 was built. Nothing caught it because the only provider that validated seeds was the one that discarded them. Fixed with `>>> 0` before the modulo; verified non-negative and still deterministic and per-shot-distinct across 40 samples. **Both bugs are the same failure at different layers: an output nobody checked, feeding an input nobody validated.**
- **Consequences:** `providers/cloudflareImage.js` gains `MODEL_INPUTS` + `buildBody()`; `IMG2IMG_SIZE` becomes `RENDER_SIZE` and is now pinned on **every** model that accepts `width`/`height`, not just the anchored path. The adapter's result reports `seed` and `negativePromptApplied` **from the body actually sent** rather than from the options requested — reporting a dropped seed is what let this run unnoticed, and FR-12 reads that field as evidence. `continuityPrompt.js:seedForShot` is corrected. **What this invalidates in the record:** DEMO-001's Cloudflare column (4/4 shots) and every earlier Cloudflare still were produced *without* the seed or the negative prompt in force. Those runs remain valid evidence that the provider works and that the images are usable; they are **not** evidence about FR-7 continuity on Cloudflare, and must not be cited as such. Pollinations, which reads both parameters, is unaffected — the FR-7 continuity results in ADR-027 were measured there and stand.
- **Verification status — deliberately partial.** The parameter table is verified against Cloudflare's live schemas (5/5 models, no invented properties), body construction is verified per model offline, and the seed fix is verified numerically. **A live end-to-end render is not yet verified:** the account's 10,000 neuron/day free allocation was exhausted during the model comparison, and every request now fails on quota (`you have used up your daily free allocation of 10,000 neurons`) rather than on schema — which does establish that the bodies pass validation and reach inference. Re-verification of a full 4-shot Cloudflare run is outstanding and must be done on a fresh day's allocation before this ADR's default is claimed proven in production. Recorded as a hold, not a retry loop, per the standing convention for quota-bound work (ADR-025).
