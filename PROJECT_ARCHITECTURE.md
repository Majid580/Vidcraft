...
| Dependencies | `backend/src/services/externalApiService.js` (BACKEND-005 — `IMPLEMENTED`, see Section 6.6) exposes one adapter per provider; `generationService.js` (ADR-021) dispatches to it per shot; Bull.js/Redis queue exists (BACKEND-003) but isn't yet wired to a real generation job (INTEG-001 — today's wiring is synchronous, ADR-014's precedent) |
...
| **Status** | `IMPLEMENTED` (ADR-021, 2026-08-11) — **deviates from the shape below**, same as `POST /api/storyboards` (ADR-014): synchronous, not async |
| **Purpose** | Kick off generation for all shots (routes each to Remotion or external API per FR-6/FR-5) |
| **Response (200)** | The full updated `Storyboard` document (not `{jobId}`) — each shot's `status`/`asset_url`/`error` reflects what actually happened: `completed` (real asset generated, `asset_url` set), `on_hold` (huggingface — deliberately not attempted, ADR-021), or `failed` (attempted, errored) |
| **WebSocket events** | Not implemented — no progress push exists yet (the response above only arrives once every shot has been attempted) |
...
| **Status** | `PROPOSED` |
| **Purpose** | Poll job status (fallback if WebSocket is unavailable) — still not needed, since generation is synchronous today |
