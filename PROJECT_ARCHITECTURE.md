...
- **Current implementation status:** `IN_PROGRESS`. The index scaffold is done + verified (RAG-001): `ai-service/rag/index.py` (`VectorIndex` — FAISS `IndexFlatIP` over L2-normalized vectors = cosine per FR-4, JSON metadata sidecar, graceful empty queries, `save`/`load`) and `ai-service/rag/embedder.py` (lazy-cached real `all-MiniLM-L6-v2`). Corpus content: `IMPLEMENTED` + verified (RAG-002): `ai-service/rag/corpus/` — 75 curated `{text, metadata}` passages (`cinematography.json`) across 8 categories (framing/angle/movement/lens/lighting/color/composition/mood), all original license-clean technique summaries per the FR-4 security note, plus a validating `load_corpus()` loader (`loader.py`) and `tests/test_corpus.py`. Populated production index: `IMPLEMENTED` + verified (RAG-003): `ai-service/rag/build_index.py` (`build_index()` + `python -m rag.build_index`) embeds the full corpus with the real encoder and persists `rag/data/style_index.{faiss,meta.json}` (384-dim, all 8 categories); idempotent and rebuilt from the committed corpus, so the index is a gitignored derived artifact (`tests/test_build_index.py`). Live-verified: reload via `VectorIndex.load()` + real semantic queries rank sensibly. Still `NOT_IMPLEMENTED`: the Cinematographer consumer (AI-007 — now fully unblocked). See PROJECT_PROGRESS.md Section 2 for the verification record.
- **Metadata sync (ADR-004 + ADR-017):** vectors live in the FAISS index; per-vector metadata (source text + dict) is the paired store. Section 10's `embeddings` collection is the eventual canonical MongoDB metadata store, synced with the FAISS index — but as of RAG-003 that sync is **still deliberately deferred (ADR-017)**: the sole consumer (AI-007) reads FAISS directly, the Python ai-service has no Mongo client, and adding an unread Mongo write would be speculative scaffolding. The JSON sidecar written beside the index (`{VECTOR_INDEX_PATH}.meta.json`) remains the canonical, self-contained, unit-testable metadata store; it holds Mongo-ready `{text, metadata}` rows so adopting the collection later is a load, not a re-embed. Revisit when a real reader of the collection exists.
- **Future work:** Corpus size/sourcing decided (RAG-002): 75 original, license-clean technique summaries across the 8 categories above, sized as a first pass — expand incrementally (then re-run `python -m rag.build_index`) if precision-at-k (§6.11) shows sparse coverage for unusual style requests. Next: AI-007 (Cinematographer consumer) queries this persisted index.
