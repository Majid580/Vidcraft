...
- **Current implementation status:** `IN_PROGRESS`. Screenwriter (AI-004), Cinematographer (AI-007), and Producer/Router (AI-005) are all implemented and verified: `POST /storyboard/generate` (ai-service) decomposes a clarified prompt into a 3-5 shot storyboard (`storyboard_id`, `world_state{characters, setting, style_tokens}`, `shots[]{shot_id, description, camera, duration_s, pathway}`), the Cinematographer then grounds each shot's `camera` and `world_state.style_tokens` in passages retrieved from the RAG-003 persisted index — replacing the Screenwriter's ADR-012 best-effort placeholder — and finally Producer/Router routes each shot's `pathway` per the ADR-013 photorealism heuristic (seeing the grounded `style_tokens`, e.g. a photorealism signal can inform its decision). Cinematographer degrades gracefully to a pass-through if the index is empty or unbuilt. Only remaining gap: the similarity-check retry loop (AI-008) does not exist yet; shots routed to `"external_api"` still cannot be rendered (BACKEND-005/PROVIDER-001 not implemented — see ADR-013 consequences).
- **Future work:** Define the exact similarity threshold (numeric value TBD, R-12) and implement AI-008's retry loop as a further graph edge/node.
