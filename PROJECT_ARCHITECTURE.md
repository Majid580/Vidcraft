...
- **Future work:** Corpus size/sourcing now decided (RAG-002): 75 original, license-clean technique summaries across the 8 categories above, sized as a first pass — expand incrementally if precision-at-k (§6.11) shows sparse coverage for unusual style requests. Next: RAG-003 (embed + persist) then AI-007 (Cinematographer consumer).
