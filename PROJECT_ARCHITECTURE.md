...
- **Current implementation status:** `IN_PROGRESS`. Four compositions exist (`remotion/src/`): `TitleCard` (generic intro/branding card) plus `WideShot`/`MediumShot`/`CloseUpShot` (keyed on shot type, per the R-11 taxonomy decision below). Each renders a real MP4 from a `Shot`/`WorldState`-shaped prop (`remotion/src/types.ts`), with duration derived from `shot.duration_s` via `calculateMetadata` — not hardcoded. A shared `theme.ts` deterministically maps `world_state.style_tokens` to a palette, so the same storyboard produces the same look across every shot (demonstrates FR-7 continuity). The automatic shot→composition *selection function* (and its fallback-default case) is still `NOT_IMPLEMENTED` — that's REMOTION-003.
- **Future work:** REMOTION-003 — implement the selection function itself (switch/lookup on `shot.camera`'s leading word → component) plus a documented fallback for shot descriptions that don't match `wide`/`medium`/`close-up`.
...
| R-11 | **Shot → Remotion composition mapping strategy undefined** | High — blocks REMOTION-003, and by extension the Minimum Viable success criterion | Partial: REMOTION-002 (2026-08-10) established a taxonomy keyed on the leading word of `shot.camera` (`wide` / `medium` / `close-up`, matching the proposal's own §6.3 example values), with one composition per key. The actual selection function (string match on `camera` → component, plus a fallback default for values that don't match any key) is still `NOT_IMPLEMENTED` — that remains REMOTION-003's job. | Design a composition taxonomy (done) + fallback default composition + selection function during Phase 4 (REMOTION-003) | Medium (down from High — the hard design question is answered, only wiring remains) | Open (partially resolved) |
