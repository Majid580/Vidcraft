...
- **Current implementation status:** `NOT_IMPLEMENTED`. Zero compositions exist.
- **Future work:** Design the composition taxonomy/library and the shot→composition mapping logic — this is one of the most concrete near-term technical unknowns in the whole project.
...
| R-11 | **Shot → Remotion composition mapping strategy undefined** | High — blocks REMOTION-003, and by extension the Minimum Viable success criterion | None | Design a composition taxonomy + fallback default composition during Phase 4 | High | Open (newly surfaced by this doc) |
